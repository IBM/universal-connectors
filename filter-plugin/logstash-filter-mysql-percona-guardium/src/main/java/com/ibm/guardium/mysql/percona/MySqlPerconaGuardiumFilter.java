...
//        }
    ...
    public Collection<Event> filter(Collection<Event> events, FilterMatchListener matchListener) {
        for (Event e : events) {
            ...
            int mysqlIndex = mes***REMOVED***geString.indexOf(MYSQL_AUDIT_START_SIGNAL);

            if (mysqlIndex != -1) {
                
                String mysqlMsgString = mes***REMOVED***geString.substring(mysqlIndex + MYSQL_AUDIT_START_SIGNAL.length());
                ...
                try {
                    JsonObject mes***REMOVED***geJson = (JsonObject) JsonParser.parseString(mysqlMsgString);
                    JsonObject audit_record = mes***REMOVED***geJson.get("audit_record").getAsJsonObject();
