...
public class GeneralParserTest {
    ...
    @Test
    public void test_applicationMes***REMOVED***ge() throws Exception {

        Event e = new org.logstash.Event();
        TestMatchListener matchListener = new TestMatchListener();

        String inputMsg = reatTestCaseInput("general_applicationMes***REMOVED***ge.txt");
        ...
        Assert.assertEquals("sentence verb", "applicationMes***REMOVED***ge" /*source.get("atype").getAsString()*/, record.getData().getConstruct().getSentences().get(0).getVerb());
        Assert.assertEquals("object type", "applicationMes***REMOVED***ge", record.getData().getConstruct().getSentences().get(0).getObjects().get(0).getType());
