...
public class MongodbGuardiumFilter implements Filter {
    ...
    public static final String LOGSTASH_TAG_SKIP_NOT_MONGODB = "_mongoguardium_skip_not_mongodb"; // skip mes***REMOVED***ges that
                                                                                                  // do not contain
                                                                                                  // "mongod:"
    /*
     * skipping non-mongo syslog mes***REMOVED***ges, and non-relevant log events like
     * "createUser", "createCollection", ... as these are already parsed in prior
     * authCheck mes***REMOVED***ges.
     */
    ...
    public Collection<Event> filter(Collection<Event> events, FilterMatchListener matchListener) {
        ...
        for (Event e : events) {
            ...
            if (e.getField("mes***REMOVED***ge") instanceof String) {
                String mes***REMOVED***geString = e.getField("mes***REMOVED***ge").toString();
                ...
                int mongodIndex = getAuditMsgStartIndex(mes***REMOVED***geString);
                if (mongodIndex != -1) {
                    String input = mes***REMOVED***geString.substring(mongodIndex + MONGOD_AUDIT_START_SIGNAL.length());
                    try {
                        ...
                        if ((!atype.equals("authCheck") && !atype.equals("authenticate")) // filter handles only authCheck mes***REMOVED***ge template & authentication error,
                            || (atype.equals("authenticate") && inputJSON.get("result").getAsString().equals("0")) // not auth success,
                            || (users.size() == 0 && !atype.equals("authenticate")) )  { // nor mes***REMOVED***ges with empty users array, as it's an internal command (except authenticate, which states in param.user)
                        ...
                        //events.remove(e);
                        log.error("MongoDB filter: Error handling mongo mes***REMOVED***ge "+input);
    ...
    private int getAuditMsgStartIndex(String mes***REMOVED***geString){
        int mongodIndex = mes***REMOVED***geString.indexOf(MONGOD_AUDIT_START_SIGNAL);
        if (mongodIndex>=0){
            log.debug("This is mongoD mes***REMOVED***ge");
            return mongodIndex;
        }
        mongodIndex = mes***REMOVED***geString.indexOf(MONGOS_AUDIT_START_SIGNAL);
        if (mongodIndex>=0){
            log.debug("This is mongoS mes***REMOVED***ge");
            return mongodIndex;
        }
        log.debug("Mes***REMOVED***ge start index was not found and mes***REMOVED***ge will be ignored");
        return -1;
    }
