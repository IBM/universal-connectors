...
class SqlParserTest {
    ...
    void testGrant() {
        ...
        grant = "GRANT SELECT (employee_id, first_name, last_name) ON employees TO hr_role WITH GRANT OPTION;";
        data = sqlParser.parseStatement(grant);
        assertNotNull(data);
        assertEquals(grant, data.getOriginalSqlCommand());
        assertEquals("GRANT", data.getConstruct().getSentences().get(0).getVerb());
        ...
        grant = "describe table extended `hive_metastore`.`default`.`testtable`";
        data = sqlParser.parseStatement(grant);
        assertNotNull(data.getConstruct());

        grant = "GRANT UPDATE, INSERT (salary, bonus) ON employees, DEPARTMENTS TO payroll_role, hr_role WITH GRANT OPTION;";
        data = sqlParser.parseStatement(grant);
        assertNotNull(data.getConstruct());
