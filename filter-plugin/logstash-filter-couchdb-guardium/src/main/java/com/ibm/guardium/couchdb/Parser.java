...
public class Parser {
	...
	public static Data parseData(final Event event){
		Data data = new Data();
		String logmes***REMOVED***ge = null ;
		if(event.getField(ApplicationConstant.LOGMESSAGE) != null) {
			logmes***REMOVED***ge = event.getField(ApplicationConstant.LOGMESSAGE).toString();
			if(logmes***REMOVED***ge.contains("?")) {
				String[] msg = logmes***REMOVED***ge.split("\\?");
				logmes***REMOVED***ge = msg[0] +"?'"+ msg[1]+"'";
			}
		}		
		data.setOriginalSqlCommand(event.getField(ApplicationConstant.VERB) != null
				? decodeContent("RESTAPI "+event.getField(ApplicationConstant.VERB).toString()+" uri=/"+event.getField(ApplicationConstant.DB_NAME).toString()+logmes***REMOVED***ge)
						: ApplicationConstant.UNKNOWN_STRING);
	...
	public static String decodeContent(String obj) {
		try {
			...
			log.error("Exception occured while decoding query: " + e.getMes***REMOVED***ge());
		}
		return obj;
	}
}
