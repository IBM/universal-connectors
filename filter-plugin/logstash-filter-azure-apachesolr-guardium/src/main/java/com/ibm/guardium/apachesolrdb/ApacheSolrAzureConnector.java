...
public class ApacheSolrAzureConnector implements Filter {
	...
	public boolean isQtpEvent(Event event) {
		if (event.getField("message") instanceof String) {
			Boolean qtp_event = (event.getField("message").toString().contains(ApplicationConstant.SOLR_QTP_LOG_UPDATE)
					&& validateQtpPost(event))
					|| (event.getField("message").toString().contains(ApplicationConstant.SOLR_QTP_REQUEST)
							&& event.getField("message").toString().contains(ApplicationConstant.STATUS_CHECK))
					|| event.getField("message").toString().contains(ApplicationConstant.ERROR)
					|| (event.getField("message").toString().contains(ApplicationConstant.SOLR_QTP_OVERSEER_STRING)
							&& event.getField("message").toString().contains(ApplicationConstant.ERROR))
					|| (event.getField("message").toString().contains(ApplicationConstant.HTTP_SOLR_CALL)
							&& event.getField("message").toString().contains(ApplicationConstant.STATUS_CHECK)
