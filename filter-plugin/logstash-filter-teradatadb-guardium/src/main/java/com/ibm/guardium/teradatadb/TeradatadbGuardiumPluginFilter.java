...
@LogstashPlugin(name = "teradatadb_guardium_plugin_filter")
public class TeradatadbGuardiumPluginFilter implements Filter {
	...
	private static Logger log = LogManager.getLogger(TeradatadbGuardiumPluginFilter.class);

	public TeradatadbGuardiumPluginFilter(String id, Configuration config, Context context) {
	...
	public Collection<Event> filter(Collection<Event> events, FilterMatchListener matchListener) {
		for (Event e : events) {
			...
			try {
					...
					log.error("Teradatadb Filter: Error Parsing Teradata Event with QueryID : {} and exception {}",
							e.getField(Constants.QUERY_ID),exception);
