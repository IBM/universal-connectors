...
class LogStash::Filters::PubsubPostgresqlGuardium < LogStash::Filters::Base
  ...
  def parsePostgresLog(event)
    begin
      mes***REMOVED***ge = event.get("textPayload").match(/(?<ts>.*)UTC\s\[(?<session_id>\d*)\].*db=(?<db_name>\S*),user=(?<uname>\S*)\s(?<severity>[A-Z]*):(?<statement>.*)/)

      statement = mes***REMOVED***ge["statement"]
      ...
      @logger.error("in parsePostgresLog: ", e.mes***REMOVED***ge)
