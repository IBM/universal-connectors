...
@LogstashPlugin(name = "***REMOVED***phana_guardium_plugin_filter")
			...
			if (e.getField(Constants.EXEC_STATEMENT) instanceof String || e.getField(Constants.ACTION_STATUS) instanceof String) {
				...
				if (setOfFingerprintID.contains(e.getField(Constants.FINGERPRINT))) {
					...
					try {
						...
						log.error("SAPHANA filter: Error parsing ***REMOVED***phana event " + exception);
						e.tag(Constants.LOGSTASH_TAG_JSON_PARSE_ERROR);
					}
				}
			} else {
				log.error("SAPHANA filter: Event has been skipped: " + e.getField("mes***REMOVED***ge")+ " originalSQL"+e.getField(Constants.EXEC_STATEMENT));
				e.tag("_guardium_skip_not_***REMOVED***phana");
			}
		}
