...
* Download the [logstash-filter-aws_opensearch_guardium_filter](https://github.com/IBM/universal-connectors/releases/download/v1.7.2/logstash-filter-opensearch_guardium_filter.zip) plug-in.

### Procedure
1. On the collector, go to ```Setup``` > ```Tools and Views``` > ```Configure Universal Connector```.
2. Enable the universal connector if it is disabled.
3. Click ```Upload File``` and select the offline  [logstash-filter-aws_opensearch_guardium_filter](https://github.com/IBM/universal-connectors/releases/download/v1.7.2/logstash-filter-opensearch_guardium_filter.zip) plug-in. After it is uploaded, click ```OK```.
