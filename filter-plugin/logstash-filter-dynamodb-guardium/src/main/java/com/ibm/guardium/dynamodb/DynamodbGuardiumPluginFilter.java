...
public class DynamodbGuardiumPluginFilter implements Filter {
	...
	public Collection<Event> filter(Collection<Event> events, FilterMatchListener matchListener) {
		// New Collection<Events> is created so as to avoid overwriting in case of
		// Collection<Events>
		Collection<Event> eventsUpdated = new ArrayList<>();
		for (Event e : events) {
			// If the input plugin used is sqs, the Events received are in the form of
			// ArrayList
			if (e.getField("message") instanceof String && e.getField("message").toString().startsWith("[")) {
				try {
					...
					String guardRecordDidNotExist = "";
					if (null != e.getField(Constants.GUARD_RECORD_DID_NOT_EXIST)) {
						guardRecordDidNotExist = String.valueOf(e.getField(Constants.GUARD_RECORD_DID_NOT_EXIST));
						log.debug("GuardRecordDidNotExist : {}", guardRecordDidNotExist);
					}
					...
					for (int i = 0; i < logArray.size(); i++) {
						...
						Map<String, Object> eventMap = convertJSONObjectToMap(inputJSON);
						org.logstash.Event newEvent = new org.logstash.Event(eventMap);

						newEvent.remove("message");

						/* If the accountId is provided in the input, we are adding it in inputJSON */
						Object accountId = e.getField("account_id");
						if (accountId != null && !String.valueOf(accountId).isEmpty()) {
							inputJSON.addProperty(Constants.ACCOUNT_ID, String.valueOf(accountId));
						}
						...
						try {
							...
							newEvent.setField(Constants.GUARD_RECORD_DID_NOT_EXIST, guardRecordDidNotExist);
