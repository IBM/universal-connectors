...
- This password will be required while executing the queries or calling the APIs.
 ...
 6. To Check that Elasticsearch is running use the below commands
    ...
    curl -XGET -u elastic:REDACTED "https://localhost:9200/?pretty" -k 
 ...
 7. For trial license run below api 
    ...
    curl -XPOST -u elastic:REDACTED " https://localhost:9200/_license/start_trial?acknowledge=true&pretty" -k

 8. To check the status of license run below api

    curl -XGET -u elastic:REDACTED "https://localhost:9200/_license?pretty" -k
...
4. To run the elasticsearch sql cli, we need to run below command.
    ...
    sudo ./bin/elasticsearch-sql-cli http://elastic:REDACTED@localhost:9200
...
Query 3:
    ...
    curl -XPOST -u elastic:REDACTED "http://localhost:9200/_sql?format=txt&pretty" -H 'Content-Type: application/json' -d'
    {
...
Query 4:
    ...
    curl -XPUT -u elastic:REDACTED"http://localhost:9200/_ingest/pipeline/my-pipeline-id?pretty" -H 'Content-Type: application/json' -d'
    {
