...
public class ApacheSolrGcpConnector implements Filter {
	 ...
	 * validated
	 * 
	 * @param mes***REMOVED***ge
	 * @return status
	 */
	public static Boolean validateQtpPost(String mes***REMOVED***ge) {
		boolean status = Boolean.FALSE;
		try {
			if (!StringUtils.isEmpty(mes***REMOVED***ge) && mes***REMOVED***ge.contains(ApplicationConstant.PARAMS)) {
				if (mes***REMOVED***ge.split(ApplicationConstant.PARAMS).length == 2) {
					String params = mes***REMOVED***ge.split(ApplicationConstant.PARAMS)[1];
	...
	public static Boolean validateCoreAction(String mes***REMOVED***ge) {
		boolean status = Boolean.FALSE;
		try {
			if (!StringUtils.isEmpty(mes***REMOVED***ge) && mes***REMOVED***ge.contains(ApplicationConstant.PARAMS)) {
				if (mes***REMOVED***ge.split(ApplicationConstant.PARAMS).length == 2) {
	...
	public boolean isQtpEvent(Event e) {
		if (e.getField("mes***REMOVED***ge") instanceof String) {
			Boolean qtp_event = e.getField("mes***REMOVED***ge").toString().contains(ApplicationConstant.SOLR_POST_QTP_MARK_STRING)
					&& validateQtpPost(String.valueOf(e.getField("mes***REMOVED***ge")))
					|| e.getField("mes***REMOVED***ge").toString().contains(ApplicationConstant.SOLR_REQUEST_QTP_MARK_STRING)
					|| (e.getField("mes***REMOVED***ge").toString()
							.contains(ApplicationConstant.SOLR_COLLECTION_ERROR_QTP_MARK_STRING)
							&& e.getField("mes***REMOVED***ge").toString().contains(ApplicationConstant.ERROR))
					|| e.getField("mes***REMOVED***ge").toString().contains(ApplicationConstant.ERROR)
					|| e.getField("mes***REMOVED***ge").toString().contains(ApplicationConstant.HTTP_SOLR_CALL)
							&& validateCoreAction(String.valueOf(e.getField("mes***REMOVED***ge")));
			return qtp_event;
		}
		return false;
	}
