...
import java.security.Mes***REMOVED***geDigest;
...
public class HdfsGuardiumFilter implements Filter {
    ...
    public Collection<Event> filter(Collection<Event> events, FilterMatchListener matchListener) {
        ...
        for (Event e : events) {
			...
			if (e.getField("mes***REMOVED***ge") instanceof String) {
				// String msg_string = e.getField("mes***REMOVED***ge").toString();
				if (e.getField("mes***REMOVED***ge").toString().contains(HdfsGuardiumFilter.HDFS_AUDIT_MARK_STRING)) {
					try {
	...
	public static short getSessionHash(String app_user, String user, String client_ip) {
		...
		short hash_id = 0;
		try {
			Mes***REMOVED***geDigest msg_digest = Mes***REMOVED***geDigest.getInstance("SHA-256");
