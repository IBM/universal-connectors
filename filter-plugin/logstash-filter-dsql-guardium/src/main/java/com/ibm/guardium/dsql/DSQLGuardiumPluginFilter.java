...
@LogstashPlugin(name = "dsql_guardium_plugin_filter")

public class DSQLGuardiumPluginFilter implements Filter {
	...
	private static Logger log = LogManager.getLogger(DSQLGuardiumPluginFilter.class);

	public DSQLGuardiumPluginFilter(String id, Configuration config, Context context) {
		...
		for (Event e : events) {
			if(null != e && null != e.getData()){
				try {
					...
					log.error("Failed to parse DSQL event: {}", exception.getMessage(), exception);
					log.warn("Failed event data: {}" , e.getData());
				}
			}
