...
public class GreenplumdbGuardiumFilter implements Filter {
			...
			if (event.getField(ApplicationConstant.MESSAGE) != null && (event.getField(ApplicationConstant.MESSAGE)
				...
				try {
					if (isParse(event)) {
						...
						final GsonBuilder builder = new GsonBuilder().di***REMOVED***bleHtmlEscaping();
					...
					LOGGER.error(e.getMes***REMOVED***ge());
	...
	private boolean isParse(Event event) {
		...
		final String trn***REMOVED***ctionID = event.getField(ApplicationConstant.TRANSACTION_ID) != null
		...
		if (trn***REMOVED***ctionID.equals("0") && !sliceID.isEmpty()) {
			flag = sliceID.equals(ApplicationConstant.SLICE) ? true : false;

		} else if (trn***REMOVED***ctionID.equals("0") || fileName.equals(ApplicationConstant.ELOG)
