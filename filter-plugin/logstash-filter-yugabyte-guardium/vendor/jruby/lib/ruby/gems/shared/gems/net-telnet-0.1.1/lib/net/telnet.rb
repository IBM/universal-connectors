...
module Net
  ...
  class Telnet
    ...
    OPT_NAMS           =   4.chr # "\004" # "\x04" # Approx Mes***REMOVED***ge Size Negotiation
    ...
    # status mes***REMOVED***ges on the attempt to connect to the server, of
    # the form:
    #
    #   Trying localhost...
    ...
    #           SGA and BIN.
    #
    # Output_log:: the name of the file to write connection status mes***REMOVED***ges
    ...
    #            printable equivalent), with connection status mes***REMOVED***ges
    ...
    #           You can di***REMOVED***ble the timeout by setting this value to false.
    ...
    #            that what it ***REMOVED***w was not really a prompt.  This is to try to
    ...
    def initialize(options) # :yield: mesg
      ...
      if @options.has_key?("Proxy")
        ...
        mes***REMOVED***ge = "Trying " + @options["Host"] + "...\n"
        yield(mes***REMOVED***ge) if block_given?
        @log.write(mes***REMOVED***ge) if @options.has_key?("Output_log")
        @dumplog.log_dump('#', mes***REMOVED***ge) if @options.has_key?("Dump_log")
        ...
        mes***REMOVED***ge = "Connected to " + @options["Host"] + ".\n"
        yield(mes***REMOVED***ge) if block_given?
        @log.write(mes***REMOVED***ge) if @options.has_key?("Output_log")
        @dumplog.log_dump('#', mes***REMOVED***ge) if @options.has_key?("Dump_log")
      end
    ...
    # is read in (not neces***REMOVED***rily all in one go), or nil if EOF
