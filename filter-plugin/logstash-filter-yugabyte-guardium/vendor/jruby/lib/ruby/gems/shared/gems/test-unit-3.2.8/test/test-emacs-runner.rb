...
class TestUnitEmacsRunner < Test::Unit::TestCase
  def test_format_failure_with_a_location
    ...
    mes***REMOVED***ge = "FAIL!!!"
    failure = Test::Unit::Failure.new(test_name, [location], mes***REMOVED***ge)
...
EOM
  ...
  def test_format_failure_with_locations
    ...
    mes***REMOVED***ge = "Many backtrace!!!"
    failure = Test::Unit::Failure.new(test_name, locations, mes***REMOVED***ge)
    assert_equal(<<-EOM.chomp, runner.send(:format_fault, failure))
Failure:
#{test_name}
#{locations.join("\n")}:
