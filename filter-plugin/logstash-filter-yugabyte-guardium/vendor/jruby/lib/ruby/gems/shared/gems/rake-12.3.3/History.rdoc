...
* Use the application's name in error mes***REMOVED***ge if a task is not found.
...
* Correctly handle bad encoding in exception mes***REMOVED***ges. Pull request #113
  by Tomer Brisker
...
* Lookup prerequisites with ***REMOVED***me name outside of scope instead of
...
* Suppress org.jruby package files in rake error mes***REMOVED***ges for JRuby users.
...
* Fixed documentation for calling tasks with arguments.  Pull request #235 by
  John Varghese.
* Clarified `rake -f` u***REMOVED***ge mes***REMOVED***ge.  Pull request #252 by Marco Pfatschbacher.
* Fixed a test failure on windows.  Pull request #231 by Hiroshi Shiro***REMOVED***ki.
...
* Rafael Ro***REMOVED*** Fu (documentation typo)
...
deprecated feeatures, feel free to use Rake 9 (0.9.3) with the ***REMOVED***me
...
* You can ***REMOVED***y "--backtrace=stdout" and "--trace=stdout" to route trace
...
* Stand alone FileList u***REMOVED***ge gets glob function (without loading in
  extra dependencies)
...
* RakeFileUtils.verbose_flag = nil silences output the ***REMOVED***me as 0.8.7
...
mes***REMOVED***ges.  This allows Rake 0.9.1 to be used with older rakefiles with
warning mes***REMOVED***ges.
...
* Rake now outputs various mes***REMOVED***ges to $stderr instead of $stdout.
...
commands on Windows.  The "sh" command now has the ***REMOVED***me semantics on
...
the ***REMOVED***me semantics on Windows that it has on Unix based platforms.
...
* Changed from using Mutex to Monitor. Evidently Mutex causes thread
  join errors when Ruby is compiled with -di***REMOVED***ble-pthreads. (Patch
...
that required a version number, I could ***REMOVED***y:
...
* Error mes***REMOVED***ges are now send to stderr rather than stdout (from
  Payton Quackenbush).
...
* Changes in the exception reported for the FileUtils.ln caused
  ***REMOVED***fe_ln to fail with a NotImplementedError.  Rake 0.7.1 will now
...
* The -f option without a value will di***REMOVED***ble the search for a
...
* James Britt and As***REMOVED***ph Mehr for reporting and helping to debug the
  ***REMOVED***fe_ln issue.
...
* Added ***REMOVED***fe_ln support for openAFS (via Ludvig Omholt).
...
For example:
  ...
  namespace "***REMOVED***mples" do
    task :build do
      # Build the ***REMOVED***mple programs
    end
  end

  task :build_all => ["main:build", "***REMOVED***mples:build"]
...
* David Heinemeier Hansson (for --trace mes***REMOVED***ge enhancement and for
...
* Ma***REMOVED***o Mutch (better support for non-ruby Gem platforms)
...
* FileUtil methods (e.g. cp, mv, install) are now declared to be
  ...
  methods of the ***REMOVED***me name.
* Fixed the verbose flag in the TestTask so that the test code is
  controlled by the flag.  Also shortened up some failure mes***REMOVED***ges.
...
* Added --silent (-s) to suppress the (in directory) rake mes***REMOVED***ge.

=== 0.4.11

* Changed the "don't know how to rake" mes***REMOVED***ge (finally)
...
=== 0.4.6
* Removed test version of ln in FileUtils that prevented ***REMOVED***fe_ln from
  using ln.
...
=== 0.4.2
* Added ***REMOVED***fe_ln that falls back to a copy if a file link is not supported.
* Package builder now uses ***REMOVED***fe\_ln.
