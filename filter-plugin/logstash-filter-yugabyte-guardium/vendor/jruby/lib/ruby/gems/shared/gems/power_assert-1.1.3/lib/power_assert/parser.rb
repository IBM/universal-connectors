  ...
  class Parser
    ...
    def extract_idents(sexp)
      ...
      when :call
        ...
        with_***REMOVED***fe_op = ((op_sym == :@op and op_name == '&.') or op_sym == :"&.")
        if method == :call
          handle_columnless_ident(extract_idents(recv), :call, [], with_***REMOVED***fe_op)
        else
          extract_idents(recv) + (with_***REMOVED***fe_op ? [Branch[extract_idents(method), []]] : extract_idents(method))
        end
      when :array
    ...
    def handle_columnless_ident(left_idents, mid, right_idents, with_***REMOVED***fe_op = false)
      ...
      if indices.length == 1 or !(right_idents.empty? and left_idents.empty?)
        ...
        left_idents + right_idents + (with_***REMOVED***fe_op ? [Branch[[ident], []]] : [ident])
      else
