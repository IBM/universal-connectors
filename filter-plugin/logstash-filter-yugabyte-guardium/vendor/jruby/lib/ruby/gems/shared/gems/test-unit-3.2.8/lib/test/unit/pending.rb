...
module Test
  module Unit
    ...
    module TestCasePendingSupport
      ...
      #     # Reached here
      #   end
      def pend(mes***REMOVED***ge=nil, &block)
        mes***REMOVED***ge ||= "pended."
        if block_given?
          pending = nil
          begin
            yield
          rescue Exception
            pending = Pending.new(name, filter_backtrace(caller), mes***REMOVED***ge,
                                  :method_name => @method_name)
            add_pending(pending)
          end
          unless pending
            flunk("Pending block should not be passed: #{mes***REMOVED***ge}")
          end
        else
          raise PendedError.new(mes***REMOVED***ge)
        end
      end
