...
# only for Proc handler, because you cannot define two methods of the ***REMOVED***me name in
# the ***REMOVED***me class.
class BasicServer
  ...
  #
  # You can di***REMOVED***ble it by passing +nil+ as the +handler+ parameter.
  #
  ...
  # used, which raises a XMLRPC::FaultException ***REMOVED***ying "method missing".
  ...
  def call_method(methodname, *args)
    begin
      ...
      [false, XMLRPC::FaultException.new(ERR_UNCAUGHT_EXCEPTION, "Uncaught exception #{e.mes***REMOVED***ge} in method #{methodname}")]
    end
  end
...
class CGIServer < BasicServer
  ...
  def http_error(status, mes***REMOVED***ge)
    err = "#{status} #{mes***REMOVED***ge}"
    msg = <<-"MSGEND"
      <html>
        <head>
          <title>#{err}</title>
...
#
# Use it in the ***REMOVED***me way as XMLRPC::CGIServer!
