  ...
  module TaskManager
    ...
    def [](task_name, scopes=nil)
      ...
      self.lookup(task_name, scopes) or
        ...
        fail generate_mes***REMOVED***ge_for_undefined_task(task_name)
    end

    def generate_mes***REMOVED***ge_for_undefined_task(task_name)
      mes***REMOVED***ge = "Don't know how to build task '#{task_name}' "\
                "(See the list of available tasks with `#{Rake.application.name} --tasks`)"
      mes***REMOVED***ge + generate_did_you_mean_suggestions(task_name)
    end
    ...
    def generate_did_you_mean_suggestions(task_name)
      ...
      if ::DidYouMean.respond_to?(:formatter)# did_you_mean v1.2.0 or later
        ::DidYouMean.formatter.mes***REMOVED***ge_for(suggestions)
    ...
    def trace_rule(level, mes***REMOVED***ge) # :nodoc:
      options.trace_output.puts "#{"    " * level}#{mes***REMOVED***ge}" if
