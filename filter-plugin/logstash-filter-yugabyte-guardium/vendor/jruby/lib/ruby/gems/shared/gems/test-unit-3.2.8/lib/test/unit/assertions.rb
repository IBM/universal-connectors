...
module Test
  module Unit
    ...
    # Notes:
    #
    # * The mes***REMOVED***ge to each assertion, if given, will be propagated with the
    #   failure.
    # * It is easy to add your own assertions based on assert_block().
    #
    # @example Example Custom Assertion
    #
    #   def deny(boolean, mes***REMOVED***ge = nil)
    #     mes***REMOVED***ge = build_mes***REMOVED***ge mes***REMOVED***ge, '<?> is not false or nil.', boolean
    #     assert_block mes***REMOVED***ge do
    #       not boolean
    #     end
    #   end
    ...
    module Assertions
      ...
      #     do_the_thing
      #   end
      def assert_block(mes***REMOVED***ge="assert_block failed.")
        _wrap_assertion do
          if (! yield)
            raise AssertionFailedError.new(mes***REMOVED***ge.to_s)
          end
        end
      end
      ...
      # @overload assert(object, mes***REMOVED***ge=nil)
      #
      ...
      #   @param [String] mes***REMOVED***ge The additional user mes***REMOVED***ge. It is
      #     showed when the assertion is failed.
      #   @return [void]
      #
      # @overload assert(mes***REMOVED***ge=nil) {}
      #
      ...
      #   @return [void]
      def assert(object=NOT_SPECIFIED, mes***REMOVED***ge=nil, &block)
        _wrap_assertion do
          have_object = !NOT_SPECIFIED.equal?(object)
          if block
            mes***REMOVED***ge = object if have_object
            if defined?(PowerAssert)
              PowerAssert.start(block, :assertion_method => __callee__) do |pa|
                pa_mes***REMOVED***ge = AssertionMes***REMOVED***ge.delayed_literal(&pa.mes***REMOVED***ge_proc)
                assertion_mes***REMOVED***ge = build_mes***REMOVED***ge(mes***REMOVED***ge, "?", pa_mes***REMOVED***ge)
                assert_block(assertion_mes***REMOVED***ge) do
                  pa.yield
                end
              end
            else
              assert(yield, mes***REMOVED***ge)
            end
          else
            unless have_object
              raise ArgumentError, "wrong number of arguments (0 for 1..2)"
            end
            assertion_mes***REMOVED***ge = nil
            case mes***REMOVED***ge
            when nil, String, Proc
            when AssertionMes***REMOVED***ge
              assertion_mes***REMOVED***ge = mes***REMOVED***ge
            else
              error_mes***REMOVED***ge = "assertion mes***REMOVED***ge must be String, Proc or "
              error_mes***REMOVED***ge += "#{AssertionMes***REMOVED***ge}: "
              error_mes***REMOVED***ge += "<#{mes***REMOVED***ge.inspect}>(<#{mes***REMOVED***ge.class}>)"
              raise ArgumentError, error_mes***REMOVED***ge, filter_backtrace(caller)
            end
            assertion_mes***REMOVED***ge ||= build_mes***REMOVED***ge(mes***REMOVED***ge,
                                                "<?> is not true.",
                                                object)
            assert_block(assertion_mes***REMOVED***ge) do
              object
      ...
      #
      # @since 2.5.3
      def refute(object, mes***REMOVED***ge=nil)
        _wrap_assertion do
          assertion_mes***REMOVED***ge = nil
          case mes***REMOVED***ge
          when nil, String, Proc
      ...
      # error mes***REMOVED***ge is generated when this one fails that tells you the
      # values of expected and actual.
      #
      # @example
      #   assert_equal 'MY STRING', 'my string'.upcase
      def assert_equal(expected, actual, mes***REMOVED***ge=nil)
        diff = AssertionMes***REMOVED***ge.delayed_diff(expected, actual)
...
<?>(?).?
EOT
          full_mes***REMOVED***ge = build_mes***REMOVED***ge(mes***REMOVED***ge, format,
                                       expected, expected.encoding.name,
                                       actual, actual.encoding.name,
                                       diff)
        else
          full_mes***REMOVED***ge = build_mes***REMOVED***ge(mes***REMOVED***ge, <<EOT, expected, actual, diff)
<?> expected but was
<?>.?
EOT
        end
        begin
          assert_block(full_mes***REMOVED***ge) { expected == actual }
        rescue AssertionFailedError => failure
          _set_failed_information(failure, expected, actual, mes***REMOVED***ge)
      ...
      def assert_raise(*args, &block)
        assert_expected_exception = Proc.new do |*_args|
          mes***REMOVED***ge, assert_exception_helper, actual_exception = _args
          expected = assert_exception_helper.expected_exceptions
          diff = AssertionMes***REMOVED***ge.delayed_diff(expected, actual_exception)
          full_mes***REMOVED***ge = build_mes***REMOVED***ge(mes***REMOVED***ge,
                                       "<?> exception expected but was\n<?>.?",
                                       expected, actual_exception, diff)
          begin
            assert_block(full_mes***REMOVED***ge) do
              expected == [] or
      ...
      # ***REMOVED***tisfies +object.instance_of?(class).
      #
      # @example
      ...
      def assert_instance_of(klass, object, mes***REMOVED***ge="")
        _wrap_assertion do
          if klass.is_a?(Array)
            klasses = klass
          else
          ...
          klass_mes***REMOVED***ge = AssertionMes***REMOVED***ge.maybe_container(klass) do |value|
            "<#{value}>"
          end
          full_mes***REMOVED***ge = build_mes***REMOVED***ge(mes***REMOVED***ge, <<EOT, object, klass_mes***REMOVED***ge, object.class)
...
EOT
      ...
      #
      # @since 3.0.0
      def assert_not_instance_of(klass, object, mes***REMOVED***ge="")
        _wrap_assertion do
          if klass.is_a?(Array)
            klasses = klass
          else
      ...
      # @example
      #   assert_nil [1, 2].uniq!
      def assert_nil(object, mes***REMOVED***ge="")
        full_mes***REMOVED***ge = build_mes***REMOVED***ge(mes***REMOVED***ge, <<EOT, object)
<?> was expected to be nil.
EOT
        assert_block(full_mes***REMOVED***ge) { object.nil? }
      end

      ##
      # Passes if +object+.kind_of?(+klass+). When +klass+ is
      # an array of classes or modules, it passes if any
      # class or module ***REMOVED***tisfies +object.kind_of?(class_or_module).
      #
      # @example
      #   assert_kind_of(Object, 'foo')                # -> pass
      #   assert_kind_of([Fixnum, NilClass], 100)      # -> pass
      #   assert_kind_of([Fixnum, NilClass], "string") # -> fail
      def assert_kind_of(klass, object, mes***REMOVED***ge="")
        _wrap_assertion do
          if klass.is_a?(Array)
            klasses = klass
          else
      ...
      # classes (and modules) do not ***REMOVED***tisfy +object.kind_of?(class_or_module).
      #
      # @example
      ...
      #
      # @since 3.0.0
      def assert_not_kind_of(klass, object, mes***REMOVED***ge="")
        _wrap_assertion do
          if klass.is_a?(Array)
            klasses = klass
          else
      ...
      def assert_respond_to(object, method, mes***REMOVED***ge="")
        _wrap_assertion do
          full_mes***REMOVED***ge = build_mes***REMOVED***ge(mes***REMOVED***ge,
                                       "<?>.kind_of\\?(Symbol) or\n" +
          ...
          assert_block(full_mes***REMOVED***ge) {object.respond_to?(method)}
        end
      end
      ...
      def assert_not_respond_to(object, method, mes***REMOVED***ge="")
        _wrap_assertion do
          full_mes***REMOVED***ge = build_mes***REMOVED***ge(mes***REMOVED***ge,
                                       "<?>.kind_of\\?(Symbol) or\n" +
                                       "<?>.respond_to\\?(:to_str) expected",
      ...
      def assert_match(pattern, string, mes***REMOVED***ge="")
        _wrap_assertion do
          ...
          full_mes***REMOVED***ge = build_mes***REMOVED***ge(mes***REMOVED***ge,
                                       "<?> was expected to be =~\n<?>.",
                                       pattern, string)
          assert_block(full_mes***REMOVED***ge) { pattern =~ string }
        end
      end

      ##
      # Passes if +actual+ .equal? +expected+ (i.e. they are the ***REMOVED***me
      # instance).
      #
      # @example
      #   o = Object.new
      #   assert_***REMOVED***me o, o
      def assert_***REMOVED***me(expected, actual, mes***REMOVED***ge="")
        full_mes***REMOVED***ge = build_mes***REMOVED***ge(mes***REMOVED***ge, <<EOT, expected, expected.__id__, actual, actual.__id__)
<?>
with id <?> was expected to be equal\\? to
<?>
with id <?>.
EOT
        assert_block(full_mes***REMOVED***ge) { actual.equal?(expected) }
      end
      ...
      # @example
      #   assert_operator 5, :>=, 4
      def assert_operator(object1, operator, object2, mes***REMOVED***ge="")
        _wrap_assertion do
          full_mes***REMOVED***ge = build_mes***REMOVED***ge(nil, "<?>\ngiven as the operator for #assert_operator must be a Symbol or #respond_to\\?(:to_str).", operator)
          assert_block(full_mes***REMOVED***ge){operator.kind_of?(Symbol) || operator.respond_to?(:to_str)}
          full_mes***REMOVED***ge = build_mes***REMOVED***ge(mes***REMOVED***ge, <<EOT, object1, AssertionMes***REMOVED***ge.literal(operator), object2)
<?> was expected to be
?
<?>.
EOT
          assert_block(full_mes***REMOVED***ge) { object1.__send__(operator, object2) }
        end
      end
      ...
      #
      # @since 3.0.0
      def assert_not_operator(object1, operator, object2, mes***REMOVED***ge="")
        _wrap_assertion do
...
EOT
        ...
        _wrap_assertion do
          ...
          begin
            ...
            if ((args.empty? && !e.instance_of?(AssertionFailedError)) ||
              ...
              failure_mes***REMOVED***ge = build_mes***REMOVED***ge(mes***REMOVED***ge, "Exception raised:\n?", e)
              assert_block(failure_mes***REMOVED***ge) {false}
            else
              raise
            end
          end
      ...
      def flunk(mes***REMOVED***ge="Flunked")
        assert_block(build_mes***REMOVED***ge(mes***REMOVED***ge)){false}
      end
      ...
      #
      # @example
      #   assert_not_***REMOVED***me Object.new, Object.new
      def assert_not_***REMOVED***me(expected, actual, mes***REMOVED***ge="")
        full_mes***REMOVED***ge = build_mes***REMOVED***ge(mes***REMOVED***ge, <<EOT, expected, expected.__id__, actual, actual.__id__)
<?>
...
EOT
      ...
      #
      # @since 2.5.3
      alias_method :refute_***REMOVED***me, :assert_not_***REMOVED***me
      ...
      def assert_not_equal(expected, actual, mes***REMOVED***ge="")
        full_mes***REMOVED***ge = build_mes***REMOVED***ge(mes***REMOVED***ge,
      ...
      def assert_not_nil(object, mes***REMOVED***ge="")
        full_mes***REMOVED***ge = build_mes***REMOVED***ge(mes***REMOVED***ge,
      ...
      def assert_not_match(regexp, string, mes***REMOVED***ge="")
        _wrap_assertion do
          ...
          full_mes***REMOVED***ge = build_mes***REMOVED***ge(mes***REMOVED***ge,
                                       ...
                                       regexp, string)
          assert_block(full_mes***REMOVED***ge) { regexp !~ string }
        end
      end
      ...
      def assert_no_match(regexp, string, mes***REMOVED***ge="")
        _wrap_assertion do
          assert_instance_of(Regexp, regexp,
                             "The first argument to assert_no_match " +
                             "should be a Regexp.")
          assert_not_match(regexp, string, mes***REMOVED***ge)
        end
      end
      ...
      class ThrowTagExtractor
        ...
        def extract_tag
          ...
          if @@have_uncaught_throw_error
            ...
            return nil unless pattern =~ @error.mes***REMOVED***ge
            tag = $1
          end
          normalize_tag(tag)
        end
      ...
      #     throw(:done)
      #   end
      def assert_throw(expected_object, mes***REMOVED***ge="", &proc)
        _wrap_assertion do
          begin
            catch([]) {}
          rescue TypeError
          ...
          begin
            ...
            full_mes***REMOVED***ge = build_mes***REMOVED***ge(mes***REMOVED***ge,
                                         ...
                                         expected_object)
            assert_block(full_mes***REMOVED***ge) {caught}
          rescue => error
      ...
      #    [1, 2].uniq
      #  end
      def assert_nothing_thrown(mes***REMOVED***ge="", &proc)
        _wrap_assertion do
      ...
      def assert_in_delta(expected_float, actual_float, delta=0.001, mes***REMOVED***ge="")
        _wrap_assertion do
          _assert_in_delta_validate_arguments(expected_float,
                                              actual_float,
                                              delta)
          full_mes***REMOVED***ge = _assert_in_delta_mes***REMOVED***ge(expected_float,
                                                  actual_float,
                                                  delta,
                                                  mes***REMOVED***ge)
          assert_block(full_mes***REMOVED***ge) do
      ...
      def assert_not_in_delta(expected_float, actual_float, delta=0.001, mes***REMOVED***ge="")
        _wrap_assertion do
          _assert_in_delta_validate_arguments(expected_float,
                                              actual_float,
                                              delta)
      ...
      def _assert_in_delta_mes***REMOVED***ge(expected_float, actual_float, delta,
                                   mes***REMOVED***ge, options={})
...
EOT
        ...
        build_mes***REMOVED***ge(mes***REMOVED***ge, format, *arguments)
      end
      ...
      def assert_in_epsilon(expected_float, actual_float, epsilon=0.001,
        ...
        _wrap_assertion do
          _assert_in_epsilon_validate_arguments(expected_float,
                                                actual_float,
                                                epsilon)
          full_mes***REMOVED***ge = _assert_in_epsilon_mes***REMOVED***ge(expected_float,
                                                    actual_float,
                                                    epsilon,
                                                    mes***REMOVED***ge)
          assert_block(full_mes***REMOVED***ge) do
      ...
      def _assert_in_epsilon_mes***REMOVED***ge(expected_float, actual_float, epsilon,
                                     mes***REMOVED***ge, options={})
        normalized_expected = expected_float.to_f
        normalized_actual = actual_float.to_f
...
EOT
      ...
      def assert_send(send_array, mes***REMOVED***ge=nil)
        _wrap_assertion do
          ...
          assert_operator(send_array.size, :>=, 2,
                          ...
                          "and a mes***REMOVED***ge name")
          format = <<EOT
<?> was expected to respond to
<?(*?)> with a true value but was
<?>.
EOT
          receiver, mes***REMOVED***ge_name, *arguments = send_array
          result = nil
          full_mes***REMOVED***ge =
            build_mes***REMOVED***ge(mes***REMOVED***ge,
                          format,
                          receiver,
                          AssertionMes***REMOVED***ge.literal(mes***REMOVED***ge_name.to_s),
                          arguments,
                          AssertionMes***REMOVED***ge.delayed_literal {result})
          assert_block(full_mes***REMOVED***ge) do
            result = receiver.__send__(mes***REMOVED***ge_name, *arguments)
            result
          end
        end
      end
      ...
      def assert_not_send(send_array, mes***REMOVED***ge=nil)
        _wrap_assertion do
          assert_instance_of(Array, send_array,
...
EOT
      ...
      def assert_boolean(actual, mes***REMOVED***ge=nil)
        _wrap_assertion do
          assert_block(build_mes***REMOVED***ge(mes***REMOVED***ge,
      ...
      def assert_true(actual, mes***REMOVED***ge=nil)
        _wrap_assertion do
          assert_block(build_mes***REMOVED***ge(mes***REMOVED***ge,
      ...
      def assert_false(actual, mes***REMOVED***ge=nil)
        _wrap_assertion do
          assert_block(build_mes***REMOVED***ge(mes***REMOVED***ge,
      ...
      def assert_compare(expected, operator, actual, mes***REMOVED***ge=nil)
        _wrap_assertion do
...
<?>.
EOT
          full_mes***REMOVED***ge = build_mes***REMOVED***ge(mes***REMOVED***ge, template,
                                       expected, actual,
                                       expected, actual)
          assert_block(full_mes***REMOVED***ge) do
      ...
      def assert_fail_assertion(mes***REMOVED***ge=nil)
        _wrap_assertion do
          full_mes***REMOVED***ge = build_mes***REMOVED***ge(mes***REMOVED***ge,
      ...
      # mes***REMOVED***ge is +expected+.
      #
      # @example
      #   assert_raise_mes***REMOVED***ge("exception") {raise "exception"}  # -> pass
      #   assert_raise_mes***REMOVED***ge(/exc/i) {raise "exception"}       # -> pass
      #   assert_raise_mes***REMOVED***ge("exception") {raise "EXCEPTION"}  # -> fail
      #   assert_raise_mes***REMOVED***ge("exception") {}                   # -> fail
      def assert_raise_mes***REMOVED***ge(expected, mes***REMOVED***ge=nil)
        _wrap_assertion do
          full_mes***REMOVED***ge = build_mes***REMOVED***ge(mes***REMOVED***ge,
                                       "<?> exception mes***REMOVED***ge was expected " +
                                       "but none was thrown.",
                                       expected)
          exception = nil
          assert_block(full_mes***REMOVED***ge) do
          ...
          actual = exception.mes***REMOVED***ge
          diff = AssertionMes***REMOVED***ge.delayed_diff(expected, actual)
          full_mes***REMOVED***ge =
            build_mes***REMOVED***ge(mes***REMOVED***ge,
                          "<?> exception mes***REMOVED***ge expected but was\n" +
      ...
      def assert_const_defined(object, constant_name, mes***REMOVED***ge=nil)
        _wrap_assertion do
          full_mes***REMOVED***ge = build_mes***REMOVED***ge(mes***REMOVED***ge,
      ...
      def assert_not_const_defined(object, constant_name, mes***REMOVED***ge=nil)
        _wrap_assertion do
          full_mes***REMOVED***ge = build_mes***REMOVED***ge(mes***REMOVED***ge,
                                       "!<?>.const_defined\\?(<?>) expected.",
                                       object, constant_name)
      ...
      def assert_predicate(object, predicate, mes***REMOVED***ge=nil)
        _wrap_assertion do
          assert_respond_to(object, predicate, mes***REMOVED***ge)
          actual = object.__send__(predicate)
          full_mes***REMOVED***ge = build_mes***REMOVED***ge(mes***REMOVED***ge,
                                       "<?>.? is true value expected but was\n" +
                                       "<?>",
                                       object,
                                       AssertionMes***REMOVED***ge.literal(predicate),
                                       actual)
          assert_block(full_mes***REMOVED***ge) do
            actual
          end
      ...
      def assert_not_predicate(object, predicate, mes***REMOVED***ge=nil)
        _wrap_assertion do
          assert_respond_to(object, predicate, mes***REMOVED***ge)
          actual = object.__send__(predicate)
          full_mes***REMOVED***ge = build_mes***REMOVED***ge(mes***REMOVED***ge,
      ...
      def assert_alias_method(object, alias_name, original_name, mes***REMOVED***ge=nil)
        _wrap_assertion do
          find_method_failure_mes***REMOVED***ge = Proc.new do |method_name|
            build_mes***REMOVED***ge(mes***REMOVED***ge,
                          "<?>.? doesn't exist\n" +
                          "(Class: <?>)",
                          object,
                          AssertionMes***REMOVED***ge.literal(method_name),
                          object.class)
          end

          alias_method = original_method = nil
          assert_block(find_method_failure_mes***REMOVED***ge.call(alias_name)) do
            begin
          ...
          assert_block(find_method_failure_mes***REMOVED***ge.call(original_name)) do
            begin
      ...
      def assert_path_exist(path, mes***REMOVED***ge=nil)
        _wrap_assertion do
          failure_mes***REMOVED***ge = build_mes***REMOVED***ge(mes***REMOVED***ge,
                                          "<?> was expected to exist",
                                          path)
          assert_block(failure_mes***REMOVED***ge) do
            File.exist?(path)
          end
        end
      end
      ...
      def assert_path_not_exist(path, mes***REMOVED***ge=nil)
        _wrap_assertion do
          failure_mes***REMOVED***ge = build_mes***REMOVED***ge(mes***REMOVED***ge,
      ...
      def assert_include(collection, object, mes***REMOVED***ge=nil)
        _wrap_assertion do
      ...
      def assert_not_include(collection, object, mes***REMOVED***ge=nil)
        _wrap_assertion do
          assert_respond_to(collection, :include?,
                            "The collection must respond to :include?.")
          full_mes***REMOVED***ge = build_mes***REMOVED***ge(mes***REMOVED***ge,
      ...
      def assert_empty(object, mes***REMOVED***ge=nil)
        _wrap_assertion do
      ...
      #   assert_not_empty([])                       # -> fail
      #   assert_not_empty({})                       # -> fail
      def assert_not_empty(object, mes***REMOVED***ge=nil)
        _wrap_assertion do
          assert_respond_to(object, :empty?,
                            "The object must respond to :empty?.")
          full_mes***REMOVED***ge = build_mes***REMOVED***ge(mes***REMOVED***ge,
      ...
      ##
      # Builds a failure mes***REMOVED***ge.  +head+ is added before the +template+ and
      # +arguments+ replaces the '?'s positionally in the template.
      def build_mes***REMOVED***ge(head, template=nil, *arguments)
        template &&= template.chomp
        return AssertionMes***REMOVED***ge.new(head, template, arguments)
      end
      ...
      def self.use_pp=(value)
        AssertionMes***REMOVED***ge.use_pp = value
      end
      ...
      def _assert_raise(assert_expected_exception, *args, &block)
        _wrap_assertion do
          ...
          assert_expected_exception.call(mes***REMOVED***ge, assert_exception_helper,
                                         actual_exception)
          actual_exception
        end
      end

      def _set_failed_information(failure, expected, actual, user_mes***REMOVED***ge)
        failure.expected = expected
        failure.actual = actual
        failure.inspected_expected = AssertionMes***REMOVED***ge.convert(expected)
        failure.inspected_actual = AssertionMes***REMOVED***ge.convert(actual)
        failure.user_mes***REMOVED***ge = user_mes***REMOVED***ge
      end

      class AssertionMes***REMOVED***ge
        @use_pp = true
        class << self
          ...
          def inspect
            if @value.is_a?(Array)
              values = @value.collect do |value|
                @formatter.call(AssertionMes***REMOVED***ge.convert(value))
              end
              "[#{values.join(', ')}]"
            else
              @formatter.call(AssertionMes***REMOVED***ge.convert(@value))
        ...
        def to_s
          mes***REMOVED***ge_parts = []
          if (@head)
            head = @head
            head = head.call if head.respond_to?(:call)
            head = head.to_s
            unless(head.empty?)
              mes***REMOVED***ge_parts << add_period(head)
            end
          end
          tail = template.result(@parameters.collect{|e| convert(e)})
          mes***REMOVED***ge_parts << tail unless(tail.empty?)
          mes***REMOVED***ge_parts.join("\n")
        end
      end
      ...
      class AssertExceptionHelper
        class WrappedException
          ...
          def inspect
            if default_inspect?
              "#{@exception.class.inspect}(<#{@exception.mes***REMOVED***ge}>)"
            else
              @exception.inspect
            end
          end
        ...
        def split_expected_exceptions(expected_exceptions)
          ...
          expected_exceptions.each do |exception_type|
            if exception_type.instance_of?(Module)
              ...
              full_mes***REMOVED***ge =
                @test_case.__send__(:build_mes***REMOVED***ge,
                                    nil,
                                    "<?> must be " +
                                    ...
                                    "or a Module",
                                    exception_type)
              @test_case.flunk(full_mes***REMOVED***ge)
            end
          end
        ...
        def fallback_exception_object_equal(expected_object, actual_exception)
          ...
          if owner == Kernel or owner == Exception
            expected_object.class == actual_exception.class and
              expected_object.mes***REMOVED***ge == actual_exception.mes***REMOVED***ge
          else
            false
          end
        end
