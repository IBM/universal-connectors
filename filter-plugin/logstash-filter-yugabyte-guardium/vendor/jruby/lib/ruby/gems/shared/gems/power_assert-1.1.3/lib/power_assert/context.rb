...
module PowerAssert
  class Context
    ...
    def initialize(base_caller_length)
      ...
      @trace_return = TracePoint.new(:return, :c_return) do |tp|
        begin
          ...
          warn "power_assert: [BUG] Failed to trace: #{e.class}: #{e.mes***REMOVED***ge}"
        end
      end
    end
    ...
    def mes***REMOVED***ge
      ...
      @mes***REMOVED***ge ||= build_assertion_mes***REMOVED***ge(@parser, @return_values).freeze
    end

    def mes***REMOVED***ge_proc
      -> { mes***REMOVED***ge }
    end
    ...
    def build_assertion_mes***REMOVED***ge(parser, return_values)
      if PowerAssert.configuration._colorize_mes***REMOVED***ge
      ...
      vals.each do |i|
        ...
        inspected_val.each_line do |l|
          ...
          lines << encoding_***REMOVED***fe_rstrip(sprintf(fmt, map_to))
        end
      end
      lines.join("\n")
    end
    ...
    def encoding_***REMOVED***fe_rstrip(str)
      str.rstrip
