...
- Replaced the `DidYouMean::SpellCheckable` module with the `DidYouMean::SpellChecker` class. This is a slower implementation but close to the model explained in [this talk](https://speakerdeck.com/yuki24/***REMOVED***ving-people-from-typos), more reu***REMOVED***ble and possibly makes it easier to expose the class as a public interface.
...
- Fixed a bug where a duplicate "did you mean?" mes***REMOVED***ge was appended each time `#to_s` is called ( [@danfinnie](https://github.com/danfinnie), [#51](https://github.com/yuki24/did_you_mean/issues/51 "Duplicate output for constants in separate gem"))
...
- Speed up Levenshtein by 50% and reduce 97% of memory u***REMOVED***ge
...
- Whitelist `#***REMOVED***fe_constantize` method from `ActiveSupport::Inflector` to avoid significant performance slowdown ( [@tleish](https://github.com/tleish), [#19](https://github.com/yuki24/did_you_mean/issues/19 "Significant Slowdown when Using Debugger"), [#20](https://github.com/yuki24/did_you_mean/pull/20 "Whitelisting ***REMOVED***fe\_constantize (ActiveSupport::Inflector) method"))
...
- Fixed a bug where did\_you\_mean unexpectedly di***REMOVED***bles [better\_errors](https://github.com/charliesome/better_errors)'s REPL
