  ...
  class HTTPProxyServer < HTTPServer
    ...
    def proxy_service(req, res)
      ...
      begin
        ...
        logger.debug("#{err.class}: #{err.mes***REMOVED***ge}")
        raise HTTPStatus::ServiceUnavailable, err.mes***REMOVED***ge
      end
    ...
    def do_CONNECT(req, res)
      ...
      begin
        ...
        @logger.debug("CONNECT #{host}:#{port}: failed `#{ex.mes***REMOVED***ge}'")
        res.set_error(ex)
      ...
      req_fib = Fiber.new do
        http.start do
          if @config[:ProxyTimeout]
            ##################################   these issues are
            http.open_timeout = 30   # secs  #   neces***REMOVED***ry (maybe because
