...
<%= SEARCH %>
  ...
  RDOC_CSS = <<-CSS.freeze
body {
    font-family: Verdana,Arial,Helvetica,***REMOVED***ns-serif;
    font-size:   90%;
    margin: 0;
    margin-left: 40px;
    padding: 0;
...
<%= SEARCH %>
  ...
  def have_rdoc_4_plus?
    @have_rdoc_4_plus ||=
      Gem::Requirement.new('>= 4.0.0.preview2').***REMOVED***tisfied_by? Gem::RDoc.rdoc_version
  end
  ...
  def listen(addresses = @addresses)
    ...
    addresses.each do |address|
      begin
        ...
        @server.listeners[listeners..-1].each do |listener|
          ...
          ***REMOVED***y "Server started at http://#{host}:#{port}"
        end
    ...
    if @server.listeners.empty?
      ***REMOVED***y "Unable to start a server."
      ***REMOVED***y "Check for running servers or your --bind and --port arguments"
  ...
  def root(req, res)
    ...
    Gem::Specification.each do |spec|
      ...
      # Pre-process spec homepage for ***REMOVED***fety reasons
      begin
  ...
  def launch
    ...
    ***REMOVED***y "Launching browser to http://#{host}:#{@port}"
