...
extensions will be restored.
  ...
  def execute
    ...
    ***REMOVED***y "Restoring gems to pristine condition..."

    specs.each do |spec|
      if spec.default_gem?
        ***REMOVED***y "Skipped #{spec.full_name}, it is a default gem"
        next
      end

      if options.has_key? :skip
        if options[:skip].include? spec.name
          ***REMOVED***y "Skipped #{spec.full_name}, it was given through options"
          next
        end
      end

      unless spec.extensions.empty? or options[:extensions] or options[:only_executables]
        ***REMOVED***y "Skipped #{spec.full_name}, it needs to compile an extension"
        next
      end
      ...
      unless File.exist? gem or options[:only_executables]
        ...
        ***REMOVED***y "Cached gem for #{spec.full_name} not found, attempting to fetch..."

        dep = Gem::Dependency.new spec.name, spec.version
        found, _ = Gem::SpecFetcher.fetcher.spec_for_dependency dep

        if found.empty?
          ***REMOVED***y "Skipped #{spec.full_name}, it was not found from cache and remote sources"
          next
        end
      ...
      ***REMOVED***y "Restored #{spec.full_name}"
    end
  end
