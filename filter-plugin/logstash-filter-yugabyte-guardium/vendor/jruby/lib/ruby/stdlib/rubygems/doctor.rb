...
class Gem::Doctor
  ...
  def doctor
    ...
    ***REMOVED***y "Checking #{@gem_repository}"

    Gem.use_paths @gem_repository.to_s

    unless gem_repository?
      ***REMOVED***y 'This directory does not appear to be a RubyGems repository, ' +
          'skipping'
      ***REMOVED***y
      return
    end
  ...
  def doctor_child(sub_directory, extension) # :nodoc:
    ...
    Dir.entries(directory).sort.each do |ent|
      ...
      ***REMOVED***y "#{action} #{type} #{sub_directory}/#{File.basename(child)}"
    end
  rescue Errno::ENOENT
    # ignore
  end
