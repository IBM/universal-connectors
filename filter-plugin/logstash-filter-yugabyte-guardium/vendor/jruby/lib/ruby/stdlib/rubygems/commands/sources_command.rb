  ...
  def add_source(source_uri) # :nodoc:
    ...
    begin
      if Gem.sources.include? source
        ***REMOVED***y "source #{source_uri} already present in the cache"
      else
        source.load_specs :released
        Gem.sources << source
        Gem.configuration.write

        ***REMOVED***y "#{source_uri} added to sources"
      end
    rescue URI::Error, ArgumentError
      ***REMOVED***y "#{source_uri} is not a URI"
      terminate_interaction 1
    rescue Gem::RemoteFetcher::FetchError => e
      ***REMOVED***y "Error fetching #{source_uri}:\n\t#{e.mes***REMOVED***ge}"
      terminate_interaction 1
    end
  end
...
Do you want to add this insecure source?
  ...
  def clear_all # :nodoc:
    ...
    unless File.exist? path
      ***REMOVED***y "*** Removed specs cache ***"
    else
      unless File.writable? path
        ***REMOVED***y "*** Unable to remove source cache (write protected) ***"
      else
        ***REMOVED***y "*** Unable to remove source cache ***"
      end
...
Since all of these sources point to the ***REMOVED***me set of gems you only need one
...
To remove a source use the --remove argument:
  ...
  def remove_source(source_uri) # :nodoc:
    unless Gem.sources.include? source_uri
      ***REMOVED***y "source #{source_uri} not present in cache"
    else
      Gem.sources.delete source_uri
      Gem.configuration.write

      ***REMOVED***y "#{source_uri} removed from sources"
    end
  end
  ...
  def update # :nodoc:
    ...
    ***REMOVED***y "source cache successfully updated"
  end
  ...
  def remove_cache_file(desc, path) # :nodoc:
    ...
    if not File.exist?(path)
      ***REMOVED***y "*** Removed #{desc} source cache ***"
    elsif not File.writable?(path)
      ***REMOVED***y "*** Unable to remove #{desc} source cache (write protected) ***"
    else
      ***REMOVED***y "*** Unable to remove #{desc} source cache ***"
    end
  end
