...
#   gem 'pg'
#
# RubyGems recommends ***REMOVED***ving this as gem.deps.rb over Gemfile or Isolate.
#
...
class Gem::RequestSet::GemDependencyAPI
  ...
  #
  #   To di***REMOVED***ble requiring any file give +false+:
  #
...
Gem dependencies file #{@path} includes git reference for both ref/branch and tag but only ref/branch is used.
  ...
  def gem_platforms(options) # :nodoc:
    ...
    platform_names.any? do |platform_name|
      ...
      VERSION_MAP[platform_name].***REMOVED***tisfied_by? Gem.ruby_version
    end
  end
  ...
  ##
  # Pins the gem +name+ to the given +source+.  Adding a gem with the ***REMOVED***me
  ...
  def ruby(version, options = {})
    ...
    unless RUBY_VERSION == version
      mes***REMOVED***ge = "Your Ruby version is #{RUBY_VERSION}, " +
                "but your #{gem_deps_file} requires #{version}"

      raise Gem::RubyVersionMismatch, mes***REMOVED***ge
    end

    if engine and engine != Gem.ruby_engine
      mes***REMOVED***ge = "Your Ruby engine is #{Gem.ruby_engine}, " +
