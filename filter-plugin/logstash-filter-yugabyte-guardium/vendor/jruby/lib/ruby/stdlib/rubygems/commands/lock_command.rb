...
class Gem::Commands::LockCommand < Gem::Command
  ...
  def initialize
    ...
    add_option '-s', '--[no-]strict',
               'fail if unable to ***REMOVED***tisfy a dependency' do |strict, options|
...
Rake-0.7.0.1 is the most recent version installed that ***REMOVED***tisfies that, so we
lock it down to the exact version.
  ...
  def complain(mes***REMOVED***ge)
    if options[:strict]
      raise Gem::Exception, mes***REMOVED***ge
    else
      ***REMOVED***y "# #{mes***REMOVED***ge}"
    end
  end

  def execute
    ***REMOVED***y "require 'rubygems'"
    ...
    until pending.empty? do
      ...
      ***REMOVED***y "gem '#{spec.name}', '= #{spec.version}'" unless locked[spec.name]
      ...
      spec.runtime_dependencies.each do |dep|
        ...
        if candidates.empty?
          complain "Unable to ***REMOVED***tisfy '#{dep}' from currently installed gems"
        else
