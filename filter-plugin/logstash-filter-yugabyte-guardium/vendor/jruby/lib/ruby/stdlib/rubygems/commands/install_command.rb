...
You can use `i` command instead of `install`.
  ...
  def install_from_gemdeps # :nodoc:
    ...
    specs = rs.install_from_gemdeps options do |req, inst|
      ...
      if inst
        ***REMOVED***y "Installing #{s.name} (#{s.version})"
      else
        ***REMOVED***y "Using #{s.name} (#{s.version})"
      end
    end
  ...
  def install_gem(name, version) # :nodoc:
    ...
    if options[:explain]
      ***REMOVED***y "Gems to install:"

      request_set.sorted_requests.each do |activation_request|
        ***REMOVED***y "  #{activation_request.full_name}"
      end
    else
  ...
  def install_gems # :nodoc:
    ...
    get_all_gem_names_and_versions.each do |gem_name, gem_version|
      ...
      begin
        ...
        exit_code |= 2
      rescue Gem::Un***REMOVED***tisfiableDependencyError => e
        show_lookup_failure e.name, e.version, e.errors, supress_suggestions,
  ...
  def load_hooks # :nodoc:
    if options[:install_as_default]
      require 'rubygems/install_default_mes***REMOVED***ge'
    else
      require 'rubygems/install_mes***REMOVED***ge'
    end
  ...
  def show_install_errors(errors) # :nodoc:
    ...
    errors.each do |x|
      ...
      msg = "Unable to pull data from '#{x.source.uri}': #{x.error.mes***REMOVED***ge}"
  ...
  def show_installed # :nodoc:
    ...
    ***REMOVED***y "#{@installed_specs.length} #{gems} installed"
  end
