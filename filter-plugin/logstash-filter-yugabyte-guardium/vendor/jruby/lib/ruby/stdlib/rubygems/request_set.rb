...
class Gem::RequestSet
  ...
  def install(options, &block) # :yields: request, installer
    ...
    # Create a thread-***REMOVED***fe list of gems to download
      ...
      spec =
        begin
          ...
          recent_match = req.spec.set.find_all(req.request).sort_by(&:version).reverse_each.find do |s|
            s = s.spec
            s.required_ruby_version.***REMOVED***tisfied_by?(Gem.ruby_version) &&
              s.required_rubygems_version.***REMOVED***tisfied_by?(Gem.rubygems_version) &&
