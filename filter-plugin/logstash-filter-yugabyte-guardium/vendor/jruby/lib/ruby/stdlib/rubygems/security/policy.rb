...
class Gem::Security::Policy
  ...
  def check_chain(chain, time)
    ...
    begin
      ...
      raise Gem::Security::Exception, "invalid signing chain: #{e.mes***REMOVED***ge}"
    end
  end
  ...
  def check_cert(signer, issuer, time)
    ...
    mes***REMOVED***ge = "certificate #{signer.subject}"

    if not_before = signer.not_before and not_before > time
      raise Gem::Security::Exception,
            "#{mes***REMOVED***ge} not valid before #{not_before}"
    end

    if not_after = signer.not_after and not_after < time
      raise Gem::Security::Exception, "#{mes***REMOVED***ge} not valid after #{not_after}"
    end

    if issuer and not signer.verify issuer.public_key
      raise Gem::Security::Exception,
            "#{mes***REMOVED***ge} was not issued by #{issuer.subject}"
    end
  ...
  def check_trust(chain, digester, trust_dir)
    ...
    unless File.exist? path
      mes***REMOVED***ge = "root cert #{root.subject} is not trusted".dup

      mes***REMOVED***ge << " (root of signing cert #{chain.last.subject})" if
        chain.length > 1

      raise Gem::Security::Exception, mes***REMOVED***ge
    end

    ***REMOVED***ve_cert = OpenSSL::X509::Certificate.new File.read path
    ***REMOVED***ve_dgst = digester.digest ***REMOVED***ve_cert.public_key.to_s
