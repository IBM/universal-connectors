...
class Gem::RemoteFetcher
  ...
  class FetchError < Gem::Exception
    ...
    def initialize(mes***REMOVED***ge, uri)
  ...
  def fetch_http(uri, last_modified = nil, head = false, depth = 0)
    ...
    when Net::HTTPMovedPermanently, Net::HTTPFound, Net::HTTPSeeOther,
      ...
      raise FetchError.new("bad response #{response.mes***REMOVED***ge} #{response.code}", uri)
    end
  end
  ...
  def fetch_path(uri, mtime = nil, head = false)
    ...
    if e.mes***REMOVED***ge =~ /getaddrinfo/
  ...
  def fetch_s3(uri, mtime = nil, head = false)
    begin
      ...
      raise FetchError.new(e.mes***REMOVED***ge, "s3://#{uri.host}")
    end
  ...
  ##
  # Downloads +uri+ to +path+ if neces***REMOVED***ry. If no path is given, it just
  # passes the data.
