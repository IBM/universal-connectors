...
use with other commands.
    ...
    if local?
      specs.concat Gem::Specification.stubs.find_all { |spec|
        name_pattern =~ spec.name and
          dependency.requirement.***REMOVED***tisfied_by? spec.version
      }.map(&:to_spec)
    end
    ...
    specs.each do |spec|
      unless spec.dependencies.empty?
        spec.dependencies.sort_by { |dep| dep.name }.each do |dep|
          ***REMOVED***y "#{dep.name} --version '#{dep.requirement}'"
        end
      end
    end
  end
  ...
  def ensure_specs(specs) # :nodoc:
    ...
    ***REMOVED***y "No gems found matching #{patterns} (#{options[:version]})" if
  ...
  ##
  # Returns an Array of [specification, dep] that are ***REMOVED***tisfied by +spec+.
    ...
    Gem::Specification.each do |sp|
      sp.dependencies.each do |dep|
        ...
        if spec.name == dep.name and
           dep.requirement.***REMOVED***tisfied_by?(spec.version)
