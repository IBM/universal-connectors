...
class Gem::RequestSet::Lockfile::Parser
  ...
  def get(expected_types = nil, expected_value = nil) # :nodoc:
    ...
    if expected_types and not Array(expected_types).include? token.type
      ...
      mes***REMOVED***ge = "unexpected token [#{token.type.inspect}, #{token.value.inspect}], " +
                "expected #{expected_types.inspect}"

      raise Gem::RequestSet::Lockfile::ParseError.new mes***REMOVED***ge, token.column, token.line, @filename
    end
