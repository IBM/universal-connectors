...
class Gem::SpecificationPolicy
  ...
  def validate_dependencies # :nodoc:
    ...
    error_mes***REMOVED***ges = []
    warning_mes***REMOVED***ges = []
    @specification.dependencies.each do |dep|
      if prev = seen[dep.type][dep.name]
        error_mes***REMOVED***ges << <<-MESSAGE
duplicate dependency on #{dep}, (#{prev.requirement}) use:
    add_#{dep.type}_dependency '#{dep.name}', '#{dep.requirement}', '#{prev.requirement}'
      ...
      warning_mes***REMOVED***ges << "prerelease dependency on #{dep} is not recommended" if
      ...
      if open_ended
        ...
        warning_mes***REMOVED***ges << ["open-ended dependency on #{dep} is not recommended", recommendation].join("\n") + "\n"
      end
    end
    if error_mes***REMOVED***ges.any?
      error error_mes***REMOVED***ges.join
    end
    if warning_mes***REMOVED***ges.any?
      warning_mes***REMOVED***ges.each { |warning_mes***REMOVED***ge| warning warning_mes***REMOVED***ge }
    end
  end
  ...
  def validate_licenses
    ...
    licenses.each do |license|
      ...
      if !Gem::Licenses.match?(license)
        ...
        mes***REMOVED***ge = <<-warning
license value '#{license}' is invalid.  Use a license identifier from
http://spdx.org/licenses or '#{Gem::Licenses::NONSTANDARD}' for a nonstandard license.
        warning
        mes***REMOVED***ge += "Did you mean #{suggestions.map { |s| "'#{s}'"}.join(', ')}?\n" unless suggestions.nil?
        warning(mes***REMOVED***ge)
      end
    end
