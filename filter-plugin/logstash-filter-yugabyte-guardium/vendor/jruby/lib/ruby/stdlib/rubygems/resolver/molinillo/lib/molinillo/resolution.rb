  ...
  class Resolver
    ...
    class Resolution
      ...
      def attempt_to_activate_existing_spec(existing_node)
        ...
        if requirement_***REMOVED***tisfied_by?(requirement, activated, existing_spec)
          new_requirements = requirements.dup
          push_state_for_requirements(new_requirements, false)
        else
          return if attempt_to_swap_possibility
          create_conflict
          debug(depth) { "Un***REMOVED***tisfied by existing spec (#{existing_node.payload})" }
          unwind_for_conflict
        end
      end
      ...
      def attempt_to_swap_possibility
        ...
        if !vertex.requirements.
           all? { |r| requirement_***REMOVED***tisfied_by?(r, activated, possibility) } ||
            !new_spec_***REMOVED***tisfied?
      ...
      # @return [void]
      def fixup_swapped_children(vertex) # rubocop:di***REMOVED***ble Metrics/CyclomaticComplexity
        ...
        vertex.outgoing_edges.each do |outgoing_edge|
          ...
          if matching_deps.empty? && !succ.root? && succ.predecessors.to_a == [vertex]
            ...
            requirements.delete_if do |r|
              ...
              # so it's ***REMOVED***fe to delete only based upon name here
      ...
      # @return [Boolean] whether the current spec is ***REMOVED***tisfied as a new
      # possibility.
      def new_spec_***REMOVED***tisfied?
        unless requirement_***REMOVED***tisfied_by?(requirement, activated, possibility)
          debug(depth) { 'Un***REMOVED***tisfied by requested spec' }
          return false
        end

        locked_requirement = locked_requirement_named(name)

        locked_spec_***REMOVED***tisfied = !locked_requirement ||
          requirement_***REMOVED***tisfied_by?(locked_requirement, activated, possibility)
        debug(depth) { 'Un***REMOVED***tisfied by locked spec' } unless locked_spec_***REMOVED***tisfied

        locked_spec_***REMOVED***tisfied
      end
      ...
      # If the {#specification_provider} ***REMOVED***ys to
