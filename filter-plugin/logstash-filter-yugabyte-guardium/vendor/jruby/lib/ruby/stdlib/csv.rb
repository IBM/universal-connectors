...
#
# This new CSV parser is m17n ***REMOVED***vvy.  The parser works in the Encoding of the IO
...
class CSV
  ...
  class Row
    ...
    #
    # This method will fetch the field value by +header+. It has the ***REMOVED***me
    ...
    # and the pair is appended.  A Hash works the ***REMOVED***me way with the key being
    ...
    #
    # Returns +true+ if this row contains the ***REMOVED***me headers and fields in the
    # ***REMOVED***me order as +other+.
    #
    def ==(other)
  ...
  class Table
    ...
    # to be CSV::Row objects.  All rows are assumed to have the ***REMOVED***me headers.
    #
    ...
    #
    # This method returns the table and is ***REMOVED***fe to chain.
    #
    def by_col!
      @mode = :col
  ...
  # the ***REMOVED***me +data+ object (tested by Object#object_id()) with the ***REMOVED***me
  # +options+.
  #
  ...
  def self.open(filename, mode="r", **options)
    ...
    # decorator
    file_opts = {univer***REMOVED***l_newline: false}.merge(options)

    begin
      f = File.open(filename, mode, file_opts)
    rescue ArgumentError => e
      raise unless /needs binmode/ =~ e.mes***REMOVED***ge and mode == "r"
      mode = "rb"
  ...
  #                                       would have the ***REMOVED***me line endings
  ...
  #                                       with the ***REMOVED***me <tt>:col_sep</tt>,
  #                                       <tt>:row_sep</tt>, and
  ...
  #                                       fields (***REMOVED***ve that the fields do not go
  #                                       through the converters).
  # <b><tt>:write_headers</tt></b>::      When +true+ and <tt>:headers</tt> is
  #                                       set, a header row will be added to the
  #                                       output.
  # <b><tt>:header_converters</tt></b>::  Identical in functionality to
  #                                       <tt>:converters</tt> ***REMOVED***ve that the
  ...
  def initialize(data, col_sep: ",", row_sep: :auto, quote_char: '"', field_size_limit:   nil,
    ...
    #
    # prepare for building ***REMOVED***fe regular expressions in the target encoding,
  ...
  def_delegators :@io, :binmode, :binmode?, :close, :close_read, :close_write,
                       ...
                       :ioctl, :i***REMOVED***tty, :path, :pid, :pos, :pos=, :reopen,
  ...
  def shift
    ...
    loop do
      ...
      if in_extended_col
        ...
        # ***REMOVED***ve fields unconverted fields, if needed...
    ...
    if @row_sep == :auto
      if [ARGF, STDIN, STDOUT, STDERR].include?(@io) or
        ...
        begin
          ...
          #
          ***REMOVED***ved_pos = @io.pos
          while @row_sep == :auto
            #
            # if we run out of data, it's probably a single line
            # (ensure will set default value)
            #
            break unless ***REMOVED***mple = @io.gets(nil, 1024)
            # extend ***REMOVED***mple if we're unsure of the line ending
            if ***REMOVED***mple.end_with? encode_str("\r")
              ***REMOVED***mple << (@io.gets(nil, 1) || "")
            end

            # try to find a standard separator
            if ***REMOVED***mple =~ encode_re("\r\n?|\n")
              @row_sep = $&
              break
            end
          end
          ...
          while ***REMOVED***ved_pos > 1024  # avoid loading a lot of data into memory
            @io.read(1024)
            ***REMOVED***ved_pos -= 1024
          end
          @io.read(***REMOVED***ved_pos) if ***REMOVED***ved_pos.nonzero?
  ...
  # headers and fields (***REMOVED***ve that the fields do not go through the converters)
  # or by reading past them to return a field row. Headers are also ***REMOVED***ved in
  ...
  def parse_headers(row = nil)
    if @headers.nil?                # header row
      @headers = case @use_headers  # ***REMOVED***ve headers
                 # Array of headers
  ...
  #
  # This method is an encoding ***REMOVED***fe version of Regexp::escape().  It will escape
