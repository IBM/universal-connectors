...
#   --do-not-reverse-lookup     di***REMOVED***ble reverse lookup
...
module UN # :nodoc:
  ...
  def help(argv, output: $stdout)
    ...
    if all
      ...
      mes***REMOVED***ges = {}
      store = proc {|msg| mes***REMOVED***ges[cmd] = msg}
    end
    open(__FILE__) do |me|
      while me.gets("##\n")
        if help = me.gets("\n\n")
          if all or argv.include?(cmd = help[/^#\s*ruby\s.*-e\s+(\w+)/, 1])
            store[help.gsub(/^# ?/, "")]
            break unless all or argv.size > mes***REMOVED***ges.size
          end
        end
      end
    end
    if mes***REMOVED***ges
      argv.each {|arg| output << mes***REMOVED***ges[arg]}
    end
  end
end
