...
class Rake::Ant
  ...
  # 1. Execute a block as a target: add_target "foo-target" { echo :mes***REMOVED***ge => "I am cool" }
  ...
  def create_project(options)
    ...
    Project.new.tap do |p|
      ...
      p.add_build_listener(DefaultLogger.new.tap do |log|
        ...
        log.emacs_mode = true
        log.mes***REMOVED***ge_output_level = output_level
        @log = log
      end)
  ...
  def generate_methods(collection)
    ...
    collection.each do |name, clazz|
      ...
      method_name = Ant.***REMOVED***fe_method_name(name)
  ...
  class << self
    ...
    def ***REMOVED***fe_method_name(element_name)
