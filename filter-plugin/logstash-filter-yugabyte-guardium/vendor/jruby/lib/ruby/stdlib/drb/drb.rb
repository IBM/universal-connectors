...
#
# Copyright (c) 1999-2003 Ma***REMOVED***toshi SEKI.  You can redistribute it and/or
# modify it under the ***REMOVED***me terms as Ruby.
#
# Author:: Ma***REMOVED***toshi SEKI
#
...
# ***REMOVED***me or a different machine.
#
# The Ruby standard library contains the core classes of the dRuby package.
# However, the full package also includes access control lists and the
# Rinda tuple-space distributed task management system, as well as a
# large number of ***REMOVED***mples.  The full dRuby package can be downloaded from
# the dRuby home page (see *References*).
#
# For an introduction and examples of u***REMOVED***ge see the documentation to the
# DRb module.
#
# == References
#
...
#    by Ma***REMOVED***toshi Seki and Makoto Inoue
#
...
#   Translation of presentation on Ruby by Ma***REMOVED***toshi Seki.
...
# process.
#
# Method calls made over dRuby behave largely the ***REMOVED***me as normal Ruby
...
#
# For more dRuby ***REMOVED***mples, see the +***REMOVED***mples+ directory in the full
# dRuby distribution.
#
...
#
#   $SAFE = 1   # di***REMOVED***ble eval() and friends
#
...
#   #
#   # Not neces***REMOVED***ry for this small example, but will be required
...
#       end
#
#       def log(mes***REMOVED***ge)
#           File.open(@filename, "a") do |f|
#               f.puts("#{Time.now}: #{@name}: #{mes***REMOVED***ge}")
#           end
#       end
#
#   end
...
#               # make the filename ***REMOVED***fe, then declare it to be so
...
# level 1.  This will di***REMOVED***ble eval() and related calls on strings
# passed across the wire.  The ***REMOVED***mple u***REMOVED***ge code given above follows
# this practice.
#
...
# a DRb::DRbMes***REMOVED***ge instance.  This uses the Marshal module to dump
...
# Normally, it uses DRb::DRbMes***REMOVED***ge internally to manage marshalling
...
# selected by the scheme 'drbunix:'.  A ***REMOVED***mple implementation over
# HTTP can be found in the ***REMOVED***mples accompanying the main dRuby
# distribution.
#
# The ID-to-object mapping component maps dRuby object ids to the
# objects they refer to, and vice ver***REMOVED***.  The implementation to use
...
# garbage-collected within this time.  Another ***REMOVED***mple implementation
# is provided in ***REMOVED***mple/name.rb in the main dRuby distribution.  This
...
# register an object using the ***REMOVED***me dRuby name.
#
module DRb
  ...
  # and DRbNameIdConv in ***REMOVED***mple/name.rb in the full drb distribution.
  #
  # JRuby Notes:
  #
  ...
  class DRbRemoteError < DRbError
    ...
    def initialize(error)
      ...
      super("#{error.mes***REMOVED***ge} (#{error.class})")
  ...
  class DRbUnknown
    ...
    # be unmarshalled.  +err+ is the error mes***REMOVED***ge that was raised
  ...
  # Handler for sending and receiving drb mes***REMOVED***ges.
  #
  ...
  class DRbMes***REMOVED***ge
    ...
    def load(soc)  # :nodoc:
      begin
        ...
        raise(DRbConnError, $!.mes***REMOVED***ge, $!.backtrace)
      end
      ...
      DRb.mutex.synchronize do
        begin
          ***REMOVED***ve = Thread.current[:drb_untaint]
          ...
          Thread.current[:drb_untaint] = ***REMOVED***ve
        end
      end
    end
  ...
  #      Send a request to +ref+ with the given mes***REMOVED***ge id and arguments.
  #      This is most easily implemented by calling DRbMes***REMOVED***ge.send_request,
  ...
  #
  # [recv_request]
  #     Receive a request from the client and return a [object, mes***REMOVED***ge,
  #     args, block] tuple.  This is most easily implemented by calling
  #     DRbMes***REMOVED***ge.recv_request, providing a stream that sits on top of
  #     the current protocol.
  # [send_reply(succ, result)]
  #     Send a reply to the client.  This is most easily implemented
  #     by calling DRbMes***REMOVED***ge.send_reply, providing a stream that sits
  ...
  # and HTTP0 in ***REMOVED***mple/http0.rb and ***REMOVED***mple/http0serv.rb in the full
  # drb distribution.
  module DRbProtocol
  ...
  class DRbTCPSocket
    ...
    def initialize(uri, soc, config={})
      ...
      @config = config
      @acl = config[:tcp_acl]
      @msg = DRbMes***REMOVED***ge.new(config)
      set_sockopt(@socket)
  ...
  class DRbObject
    ...
    def method_missing(msg_id, *a, &b)
      ...
      succ, result = self.class.with_friend(@uri) do
        DRbConn.open(@uri) do |conn|
          conn.send_mes***REMOVED***ge(self, msg_id, a, b)
        end
      end
    ...
    def self.with_friend(uri) # :nodoc:
      ...
      ***REMOVED***ve = Thread.current['DRb']
      Thread.current['DRb'] = { 'server' => friend }
      return yield
    ensure
      Thread.current['DRb'] = ***REMOVED***ve if friend
    end
  ...
  class DRbConn
    ...
    def send_mes***REMOVED***ge(ref, msg_id, arg, block)  # :nodoc:
  ...
  class DRbServer
    ...
    # Set the default ***REMOVED***fe level to +level+.  The default ***REMOVED***fe level is 0
    #
    # See #new for more information.
    def self.default_***REMOVED***fe_level(level)
      @@***REMOVED***fe_level = level
    end
    ...
    def self.make_config(hash={})  # :nodoc:
      default_config = {
        ...
        :***REMOVED***fe_level => @@***REMOVED***fe_level
      }
    ...
    # :load_limit :: the maximum mes***REMOVED***ge size in bytes accepted by
    #                the server.  Defaults to 25 MB (26214400).
    # :argc_limit :: the maximum number of arguments to a remote
    #                method accepted by the server.  Defaults to
    #                256.
    # :***REMOVED***fe_level :: The ***REMOVED***fe level of the DRbServer.  The attribute
    ...
    def initialize(uri=nil, front=nil, config_or_acl=nil)
      ...
      @***REMOVED***fe_level = @config[:***REMOVED***fe_level]
    ...
    # The ***REMOVED***fe level for this server.  This is a number corresponding to
    # $SAFE.
    #
    # The default ***REMOVED***fe_level is 0
    attr_reader :***REMOVED***fe_level
    ...
    class InvokeMethod  # :nodoc:
      def initialize(drb_server, client)
        ...
        @***REMOVED***fe_level = drb_server.***REMOVED***fe_level
        @client = client
      end
      ...
      def perform
        @result = nil
        @succ = false
        setup_mes***REMOVED***ge

        if $SAFE < @***REMOVED***fe_level
          info = Thread.current['DRb']
          if @block
            @result = Thread.new {
              Thread.current['DRb'] = info
              $SAFE = @***REMOVED***fe_level
              perform_with_block
            }.value
          else
            @result = Thread.new {
