...
module MakeMakefile
  ...
  $univer***REMOVED***l = /univer***REMOVED***l/ =~ RUBY_PLATFORM
  ...
  module Logging
    ...
    def self::postpone
      ...
      open do
        log, ****REMOVED***ve = @log, @logfile, @orgout, @orgerr
        ...
        begin
          ...
          @log, @logfile, @orgout, @orgerr = log, ****REMOVED***ve
          @postpone -= 1
  ...
  def log_src(src, heading="checked program was")
    ...
    Logging::mes***REMOVED***ge <<"EOM"
#{heading}:
/* begin */
EOM
    src.each_with_index {|line, no| Logging::mes***REMOVED***ge fmt, no+1, line}
    Logging::mes***REMOVED***ge <<"EOM"
/* end */
...
MSG
  ...
  def cpp_command(outfile, opt="")
    conf = RbConfig::CONFIG.merge('hdrdir' => $hdrdir.quote, 'srcdir' => $srcdir.quote,
                                  'arch_hdrdir' => $arch_hdrdir.quote,
                                  'top_srcdir' => $top_srcdir.quote)
    if $univer***REMOVED***l and (arch_flag = conf['ARCH_FLAG']) and !arch_flag.empty?
...
SRC
  ...
  def checking_for(m, fmt = nil)
    ...
    mes***REMOVED***ge(a)
    Logging::mes***REMOVED***ge "--------------------\n\n"
    r
  end

  def checking_mes***REMOVED***ge(target, place = nil, opt = nil)
  ...
  def have_macro(macro, headers = nil, opt = "", &b)
    checking_for checking_mes***REMOVED***ge(macro, headers, opt) do
  ...
  def have_library(lib, func = nil, headers = nil, opt = "", &b)
    ...
    checking_for checking_mes***REMOVED***ge(func.funcall_style, LIBARG%lib, opt) do
  ...
  def find_library(lib, func, *paths, &b)
    ...
    checking_for checking_mes***REMOVED***ge(func.funcall_style, LIBARG%lib) do
      libpath = $LIBPATH
      libs = append_library($libs, lib)
      begin
  ...
  def have_func(func, headers = nil, opt = "", &b)
    checking_for checking_mes***REMOVED***ge(func.funcall_style, headers, opt) do
      if try_func(func, $libs, headers, opt, &b)
        $defs << "-DHAVE_#{func.***REMOVED***ns_arguments.tr_cpp}"
        true
      else
        false
      end
  ...
  def have_var(var, headers = nil, opt = "", &b)
    checking_for checking_mes***REMOVED***ge(var, headers, opt) do
  ...
  # file which is named ***REMOVED***me as the framework.
  ...
  def find_header(header, *paths)
    mes***REMOVED***ge = checking_mes***REMOVED***ge(header, paths)
    header = cpp_include(header)
    checking_for mes***REMOVED***ge do
  ...
  def have_struct_member(type, member, headers = nil, opt = "", &b)
    checking_for checking_mes***REMOVED***ge("#{type}.#{member}", headers) do
...
SRC
  ...
  def have_type(type, headers = nil, opt = "", &b)
    checking_for checking_mes***REMOVED***ge(type, headers, opt) do
  ...
  def find_type(type, opt, *headers, &b)
    ...
    checking_for checking_mes***REMOVED***ge(type, nil, opt), fmt do
      headers.find do |h|
...
SRC
  ...
  def have_const(const, headers = nil, opt = "", &b)
    checking_for checking_mes***REMOVED***ge([*const].compact.join(' '), headers, opt) do
  ...
  def check_sizeof(type, headers = nil, opts = "", &b)
    ...
    checking_for checking_mes***REMOVED***ge("size of #{type}", headers), fmt do
  ...
  # _convertible_ means actually the ***REMOVED***me type, or typedef'd from the ***REMOVED***me
  # type.
  #
  ...
  #   to convert +type+ to an Integer object, and vice ver***REMOVED***.
  #
...
SRC
  ...
  def what_type?(type, member = nil, headers = nil, &b)
    ...
    checking_for checking_mes***REMOVED***ge(m, headers), fmt do
  ...
  def find_executable(bin, path = nil)
    checking_for checking_mes***REMOVED***ge(bin, path) do
  ...
  # <tt>--di***REMOVED***ble-</tt>_config_ option. Returns +true+ if the enable option is
  # given, +false+ if the di***REMOVED***ble option is given, and the default value
  # otherwise.
  #
  # This can be useful for adding custom definitions, such as debug
  # information.
  ...
  def enable_config(config, default=nil)
    if arg_config("--enable-"+config)
      true
    elsif arg_config("--di***REMOVED***ble-"+config)
      false
    elsif block_given?
      yield(config, default)
    else
  ...
  def pkg_config(pkg, option=nil)
    ...
    if get and option
      ...
      Logging::mes***REMOVED***ge "package configuration for %s\n", pkg
      Logging::mes***REMOVED***ge "cflags: %s\nldflags: %s\nlibs: %s\n\n",
                       cflags, ldflags, libs
      [cflags, ldflags, libs]
    else
      Logging::mes***REMOVED***ge "package configuration for %s is not found\n", pkg
      nil
    end
  end
...
RULES
  ...
  #
  # The +srcprefix+ should be used when your source files are not in the ***REMOVED***me
  # directory as your build script. This will not only eliminate the need for
  # you to manually copy the source files into the ***REMOVED***me directory as your
  ...
  def create_makefile(target, srcprefix = nil)
    ...
    mes***REMOVED***ge "creating Makefile\n"
    MakeMakefile.rm_f "#{CONFTEST}*"
...
site-install-rb: install-rb
  ...
  FailedMes***REMOVED***ge = <<MESSAGE
Could not create Makefile due to some reason, probably lack of neces***REMOVED***ry
...
MESSAGE
  ...
  # the script will abort with an error mes***REMOVED***ge.
  #
  # Internal use only.
  #
  def mkmf_failed(path)
    unless $makefile_created or File.exist?("Makefile")
      ...
      abort "*** #{path} failed ***\n" + FailedMes***REMOVED***ge + opts.join
    end
  end
