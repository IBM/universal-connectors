...
/**
 * Handles an error.
 * Shows the mes***REMOVED***ge for the latest error then exits.
 */
void handleError () {
  LPVOID lpMsgBuf;
  int error = GetLastError();

  FormatMes***REMOVED***ge(
  ...
  /* Display the string. */
  Mes***REMOVED***geBox( NULL, (LPCTSTR)lpMsgBuf, "Error", MB_OK | MB_ICONERROR );
 ...
 * Sends a stdin-eof chunk if neces***REMOVED***ry.
 *
...
/**
 * Displays u***REMOVED***ge info and bails
 */
void u***REMOVED***ge(int exitcode) {
  fprintf(stderr, "NailGun v%s\n\n", NAILGUN_VERSION);
  fprintf(stderr, "U***REMOVED***ge: ng class [--nailgun-options] [args]\n");
  ...
  fprintf(stderr, "   --nailgun-help              print this mes***REMOVED***ge and exit\n");
...
int main(int argc, char *argv[], char *env[]) {
  ...
  /* if executing just the ng client with no arguments or -h|--help, then
     display u***REMOVED***ge and exit.  Don't handle -h|--help if a command other than
     ng or ng.exe was used, since the appropriate nail should then handle
     --help. */
  if (cmd == NULL && 
        (argc == 1 || 
	  (argc == 2 && strcmp("--help", argv[1]) == 0) ||
	  (argc == 2 && strcmp("-h", argv[1]) == 0))) u***REMOVED***ge(0);
  ...
  for (i = 1; i < argc; ++i) {
    if (!strcmp("--nailgun-server", argv[i])) {
      if (i == argc - 1) u***REMOVED***ge(NAILGUN_BAD_ARGUMENTS);
      ...
      /* just verify u***REMOVED***ge here.  do the rest when sending args. */
      if (i == argc - 1) u***REMOVED***ge (NAILGUN_BAD_ARGUMENTS);
  ...
  /* if there's no command, we should only display u***REMOVED***ge info
     if the version number was not displayed. */
  if (cmd == NULL) {
    u***REMOVED***ge(NAILGUN_BAD_ARGUMENTS);
  }
