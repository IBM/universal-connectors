...
### Meet Yugabyte
* Tested versions: 2.25.1.0
* Environment: On-premise
* Supported inputs: syslog (push)
* Supported Guardium versions:
    * Guardium Data Protection 12.1 patch 5008 and above
    * Guardium Data Protection 12.2 patch 5008 and above
    * Guardium Data Protection 12.3 and above
...
* Locate "input" in the `/etc/rsyslog.d/30-yugabyte.conf` file, then add the following parameters.
```
    input(type="imfile"
    ...
    # Template: prepend hostname,server_ip, then the original message
    template(name="YugabyteCsv"
             type="string"
             string="%hostname%,%fromhost-ip%,%msg%\n")
    
    # Forward to Guardium UC on TCP 5514 using the CSV template
    local0.* @@<target host IP>:5514;YugabyteCsv
```
...
3. Click ```Upload File``` and select the
   offline [logstash-filter-yugabytedb_guardium_filter.zip](https://github.com/IBM/universal-connectors/releases/download/main_dev/logstash-filter-yugabytedb_guardium_filter.zip)
...
6. Update the input section to add the details from
   the [syslog.conf](https://github.com/IBM/universal-connectors/raw/main/input-plugin/logstash-input-tcp-syslog/TCPInputPackage/TCP/input.conf)
...
7. Update the filter section to add the details from
   the [yugabyteFilebeat.conf](https://github.com/IBM/universal-connectors/raw/main/filter-plugin/logstash-filter-yugabyte-guardium/YugabyteOverSyslogPackage/YugabyteDB/filter.conf)
