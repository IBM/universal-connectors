...
There are multiple ways to install a MySQL server. For this example, we will assume that we already have a working MySQL setup.
...
2. First enable the Universal Guardium connector, if it is disabled already.4. Click the Plus sign to open the Connector Configuration dialog box.
