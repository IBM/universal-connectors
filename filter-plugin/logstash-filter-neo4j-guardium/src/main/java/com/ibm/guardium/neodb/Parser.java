...
public class Parser {
    ...
    Sentence parseQuery(String query) {
        List<String> gfg =
                new ArrayList<>(
                        Arrays.asList(
                                Constants.ON_CREAT_ST,
                                Constants.ON_MATC_ST,
                                Constants.MATCH,
                                Constants.CREATE,
                                Constants.MERGE,
                                Constants.DELETE,
                                Constants.DETACH_DELET,
                                Constants.REMOVE,
                                Constants.SET,
                                Constants.RETURN,
                                Constants.WHERE,
        ...
        for (String queryString : queries) {
            if (queryString.startsWith(Constants.RETURN)
                    || queryString.startsWith(Constants.WHERE)
                    || queryString.startsWith(Constants.FOREACH)) {
                continue;
            } else if (queryString.startsWith(Constants.ON_CREAT_ST)) onCreateSet(queryString);
            else if (queryString.startsWith(Constants.ON_MATC_ST)) onMatchSet(queryString);
            else if (queryString.startsWith(Constants.MATCH)) match(queryString);
            else if (queryString.startsWith(Constants.CREATE)) create(queryString);
            else if (queryString.startsWith(Constants.MERGE)) merge(queryString);
            else if (queryString.startsWith(Constants.SET)) set(queryString);
            else if (queryString.startsWith(Constants.DELETE)) delete(queryString);
            else if (queryString.startsWith(Constants.REMOVE)) remove(queryString);
            else if (queryString.startsWith(Constants.DETACH_DELET)) detachDelete(queryString);
        }

        // Form the sentences out of different verb and objects

        // convert to ArrayList of key set
        List<String> alKeys = new ArrayList<>(operations.keySet());
            ...
            for (String keys : alKeys) {
                ...
                for (String string : operation) {
                    if (j == 1) {
                        ...
                        if (sentenceObject.getName() == null) sentenceObject.setName(Constants.EVERYTHING);
                        sentenceObject.setType(
                                Constants.TYPE); // In graph database, graphs are equivalent to tables in RDBMS
