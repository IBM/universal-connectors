...
3. In the `Configuration notes` page, click `Download certificate` to download the UC certificate authority to your local system. Copy the certificate to the data source (it will be added to filebeat configuration later). All data sources of any one specific type use the same certificate.
...
*   **Cloudwatch Log Group name** - specify the log group that is created for your data instance.
 ...
 ## Configuring Filebeat to forward audit logs to Guardium
  ...
  ## Procedure
prerequisites:
* A connection was added by the steps in the [Filebeat input plug-in configuration](https://github.com/IBM/universal-connectors/blob/main/docs/Guardium Insights/3.2.x/UC_Configuration_GI.md#filebeat-input-plug-in-configuration) section.
