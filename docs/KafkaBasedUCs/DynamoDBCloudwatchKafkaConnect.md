...
7. Set the **Role Name**. For example, **Export-DynamoDB-CloudWatch-to-SQS-Lambda**. Then click **Create role**.
...
5. Set **Function name**. For example, **Export-DynamoDB-CloudWatch-Logs-To-SQS**.
6. Under **Runtime**, select **Python 3.x**.
7. Under **Permissions**, select **Use an existing role** and select the IAM role created in the previous step (Export-DynamoDB-CloudWatch-to-SQS-Lambda).
...
| **Cluster name**                        | A logical name for your DynamoDB data source. This value is not used to establish the CloudWatch connection, but it identifies the data source in Guardium activity reports: it is used as the **Server Host Name** (`<accountId>:<clusterName>`) and as the **Database Name** in Guardium reports. Use a descriptive identifier for your DynamoDB workload (e.g., `my-dynamodb-source`). |
