...
| **Cluster name**                        | The name of your RDS PostgreSQL cluster or DB instance identifier. This value is not used to establish the CloudWatch connection, but it identifies the data source in Guardium activity reports: it is used as the **Server Host Name** (`<accountId>:<clusterName>`) and as the **Database Name** in Guardium reports. Use your RDS instance identifier as shown in the AWS console (e.g., `my-postgres-instance`). |
