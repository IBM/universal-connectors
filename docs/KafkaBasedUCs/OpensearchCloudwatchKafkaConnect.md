...
| **Cluster name**                        | The name of your OpenSearch domain. This value is not used to establish the CloudWatch connection, but it identifies the data source in Guardium activity reports: it is used as the **Server Host Name** (`opensearch.aws.com-<accountId>:<clusterName>`) and as the fallback **Database Name** when the audit log does not contain a cluster name. Use your OpenSearch domain name (e.g., `my-opensearch-domain`). |
