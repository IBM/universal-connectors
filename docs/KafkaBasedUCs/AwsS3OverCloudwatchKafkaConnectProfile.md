...
| **Cluster name**                        | A logical name for your S3 data source. This value is not used to establish the CloudWatch connection, but it identifies the data source in Guardium activity reports: it is used as the **Server Host Name** (`<accountId>:<clusterName>`) and as the **Database Name** in Guardium reports. Use your S3 bucket name or a descriptive identifier (e.g., `my-s3-audit-bucket`). |
