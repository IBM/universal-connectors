...
1. **Data Ingestion Delay**: There will be a delay in data being observed for reports due to limitations of the Oracle RDS DB instance and CloudWatch log availability. The end-to-end latency from query execution to visibility in Guardium is typically **15–30 minutes**, broken down as follows:

   | Stage | Typical latency |
   |---|---|
   | Query executes → XML file written to RDS disk | < 5 seconds |
   | XML file → CloudWatch log group (RDS export agent) | 5–45 minutes (AWS-controlled, not configurable) |
   | CloudWatch → Guardium UC connector | Up to the configured **Ingestion delay** (default 45 minutes) |

   The ingestion delay is a deliberate buffer to ensure CloudWatch has fully finalized the log stream before the connector reads it. Reducing it below 15 minutes risks missing events still being ingested by AWS. On high-traffic databases, RDS rotates XML audit files more frequently, which causes the export agent to deliver logs to CloudWatch faster — often within 5–10 minutes. In those cases, reducing the ingestion delay to 15–20 minutes provides lower visibility latency without risk of data loss. On low-traffic databases where audit files rotate infrequently, the default of 45 minutes is appropriate.
...
| **Log groups**                          |         | List of CloudWatch log groups to monitor. These are the log groups where Oracle audit logs are exported. Format: `/aws/rds/instance/<instance-id>/audit`.                                                                                                                                                             |
| **Filter pattern**                      | `None`  | CloudWatch Logs filter pattern to apply. Use `None` to retrieve all logs, or specify a pattern to filter specific log events.                                                                                                                                                                                         |
| **Account ID**                          |         | Your AWS account ID (12-digit number). This identifies your AWS account.                                                                                                                                                                                                                                              |
| **Database cluster name**               |         | The name of your RDS Oracle cluster or DB instance identifier. This value is not used to establish the CloudWatch connection, but it identifies the data source in Guardium activity reports: it is used as the **Server Host Name** (`<accountId>:<clusterName>`) and as the **Database Name** in Guardium reports. Use your RDS instance identifier as shown in the AWS console (e.g., `my-oracle-rds-instance`). |
| **Ingestion delay (minutes)**           | `45`    | The delay in minutes before the connector reads CloudWatch logs. This accounts for the RDS export agent delay and CloudWatch log finalization time. The appropriate value depends on database traffic volume: high-traffic databases rotate XML audit files frequently, causing faster CloudWatch delivery (5–10 minutes), and a lower value such as 15–20 minutes can be used. Low-traffic databases rotate files infrequently, and the default of 45 minutes is appropriate. Values below 15 minutes are not recommended.                                                                   |
| **Number of events**                    | `1000`  | Maximum number of log events fetched per CloudWatch API call. The connector automatically reduces this value on AWS API throttling and recovers it gradually on successive successful polls.                                                                                                                            |
| **Poll interval (seconds)**             | `30`    | Frequency in seconds at which the connector polls CloudWatch for new log events.                                                                                                                                                                                                                                      |
| **No-traffic threshold (minutes)**      | `60`    | Duration of inactivity in minutes after which the S-TAP status turns red. The status returns to green once audit events are received again.                                                                                                                                                                           |
| **Start time (epoch in ms)**            | `0`     | When set to a non-zero value, the connector begins reading CloudWatch logs from that epoch timestamp in milliseconds. A value of `0` starts from the current time minus the ingestion delay.                                                                                                                          |
| **Unmask sensitive value**              | `false` | When enabled, sensitive values in the audit logs are not masked by AWS CloudWatch.                                                                                                                                                                                                                                    |
