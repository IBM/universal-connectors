...
Send the log to Guardium collector (or pull it): Configure sending the audit mes***REMOVED***ges/events to the Guardium collector by using Syslog or FileBeat, or another method.

2. Create a data parser plug-in for the Guardium univer***REMOVED***l connector (a Logstash filter plug-in, de facto) that parses the audit log data and prepare it for Guardium. Plug-in development and testing can be done on your dev server, without requiring a Guardium system.

3. Enable Guardium Univer***REMOVED***l connector on your Guardium system, install your plug-in on the Guardium Univer***REMOVED***l connector, and configure a Connector to test that data is flowing from your data source into Guardium.

## Procedure

1. **Configuring the native audit on the data source**.

Configure your database server to enable native auditing for your data source, and to send the mes***REMOVED***ges to Guardium. For example, via Syslog, or by installing and configuring Elastic Filebeat.
...
Activating audit logs might be different for every data source type. Give yourself some time to research the data source and activate the correct log. Then, familiarize yourself with the log configuration options, running commands, and the resulting mes***REMOVED***ges and events that you see in the audit log.

Points to consider:

* Which log mes***REMOVED***ges are triggered by your actions, and which are triggered sporadically or internally by the data source? Consider filtering out things that do not seem to be important.
...
Other fields, like server IP or host, can be populated into the Logstash Event that reaches your Logstash filter plug-in by configuring FileBeat, Syslog, or the Logstash configuration file itself. (You can apply the ***REMOVED***me configuration in the Guardium UI, later). Refer to MongoDB auditing by using Filebeat connector template to see how a ***REMOVED***mple configured server host and IP.

b. **Send or get the data from the data source**

If you want your data source server to send the log events to your Elastic Logstash (used for testing) or Guardium univer***REMOVED***l connector, configure Elastic Filebeat or Syslog to forward the events. If your data source solely exists on the cloud, you might need to find other solutions to send the audit log events or files to your Logstash or Guardium univer***REMOVED***l connector. For example, CloudWatch and CloudTrail services on Amazon AWS are another solution.

Temporarily, stop configuring the data source, and start developing something based on a few log examples you can copy from the log. That's your main focus after all. Return to this step when you're ready to install your parser on Logstash or the Guardium univer***REMOVED***l connector, and test it.

Not all data sources allow the data to be sent to another server easily, and sometimes you need to develop a Logstash filter plug-in that connects and fetches the log in chunks. For example, with Amazon S3 you can use Amazon CloudWatch, to allow fetching of the log by the Guardium Univer***REMOVED***l Connector. Other data sources (like Snowflake and Teradata) might require you to fetch activity data by using a Logstash JDBC input plugin.

i. **Configuring Syslog**

The easiest way to get started is to use Syslog, since it comes preinstalled in Linux distributions. Consider switching to Filebeat later on though, as Syslog might truncate long log mes***REMOVED***ges. In addition, Filebeat has other advantages like load balancing and better performance.
...
To configure Syslog to send the audit logs to the Guardium Univer***REMOVED***l Connector:
 ...
 Use the prefix ```@<IP:port>``` to send the logs via UDP and``` @@<IP:port>``` to send the logs via TCP. You need to verify that the ***REMOVED***me port is set in Guardium, later, when you configure the Guardium univer***REMOVED***l connector.
 2. If you want to temporarily skip sending data to a remote testing Logstash or Guardium server, configure Syslog to write the mes***REMOVED***ges to a local log file:

  ```:programname, isequal, "mongod" /var/log/syslog-mes***REMOVED***ges-mongodb.log```

 Then you can see what a typical Syslog “mes***REMOVED***ge” looks like when it reaches Logstash or Guardium Univer***REMOVED***l Connector.
...
To configure Filebeat to send the audit logs to the Guardium univer***REMOVED***l connector:
...
-If you intend to use Filebeat to send different data sources from the ***REMOVED***me server on (***REMOVED***me port), attach a different tag to each input log. The tags help later on to identify and filter or parse only the correct events that match that data source, when you configure a connection in Guardium Univer***REMOVED***l Connector (see Adding connectors and plug-ins in Guardium.) For example,
```
# ============================== Filebeat inputs ===============================
filebeat.inputs:
...
-Configure the ```output.logstash``` section. Make sure that this part is not commented out, and enter your testing Logstash or Guardium univer***REMOVED***l connector as a host.For example,
```
output.logstash:
# The Logstash hosts
hosts: ["<Univer***REMOVED***l-Connector-IP>:<port>"]
```
...
* Not all incoming events are neces***REMOVED***rily relevant. You need to find a way to filter them out at the data source server, or if not possible, then in your Filter plug-in. If you use Syslog to send data to your Logstash or Guardium Univer***REMOVED***l connector, other events might come in from other services that use the Syslog service, for example, kernel, telnet.
...
The new plug-in can be implemented in any language Logstash supports for plug-ins, like Ruby and Java. The Guardium univer***REMOVED***l connectors were developed in Java, since it is more flexible and better supports more complex projects.

To develop a plug-in for Guardium univer***REMOVED***l connector, Develop a Logstash Filter plug-in that parses the events you get from Logstash input stage, and adds a Guardium record field to them. Guardium record is a data structure that the Guardium univer***REMOVED***l connector can digest. For example, an event that is processed by your filter plug-in can look like this:
```
"GuardRecord" =>
...
```
The Guardium record, which is added to Logstash Event after the filter, is examined and handled by Guardium Univer***REMOVED***l connector (in an output stage) and inserted into Guardium.

Clone the [guardium-univer***REMOVED***lconnector-commons project](https://github.com/IBM/guardium-univer***REMOVED***lconnector-commons) from GitHub to get helper classes for creating a Guardium record.

For a production-ready example, see the [MongoDB-Guardium filter plug-in project on GitHub](https://github.com/IBM/logstash-filter-mongodb-guardium). It transforms MongoDB activity audit log mes***REMOVED***ges into a record that contains details about who performed the action, from where, and information about the command that was run. You can use it to learn how events are parsed and tested, and more. The ```README.md``` contains a brief introduction as to how the filter works, what it sends to Guardium in the end (a Record POJO), how to build and install it as a Logstash filter plug-in.

If you need to create and store files from your plug-in’s code, use the environment variable ```$THIRD_PART_PATH``` from within your plug-in, to create a persistent storage location for you to work with. For example, to manage a queue and track the latest fetched log entry. For other u***REMOVED***ges of this folder, see [Installing and testing the filter or input plug-in on a staging Guardium system].
...
* Events sent to a Logstash can differ a bit from what you see in the logs. The best way to see an incoming Logstash event mes***REMOVED***ge is to direct traffic from your data source to a Logstash service (either local or remote).

* To improve your filter plug-in performance, test that the mes***REMOVED***ges match your filter as fast soon as possible. For example, the MongoDB parser/filter uses mongod:" and mongos: (based on the Syslog template) to recognize MongoDB audit log events, instead of parsing the whole JSON structure.

* Use a specific package name for your plug-in, since Logstash loads classes by using the ***REMOVED***me class loader at the moment. This prevents clashes with other plug-ins that might have the ***REMOVED***me Class name as yours.
...
To run Logstash with your plug-in, create a simple Logstash configuration file that generates or simulates an audit mes***REMOVED***ge of your data source, then passes it to your filter.
...
filter {
  ...
  if [type] == "syslogMongoDB" {
        grok { match => { "mes***REMOVED***ge" => "%{SYSLOGTIMESTAMP:syslog_timestamp} %{SYSLOGHOST:server_hostname} %{SYSLOGPROG:source_program}(?:\[%{POSINT:syslog_pid}\])?: %{GREEDYDATA:syslog_mes***REMOVED***ge}" }
        }
        date {
...
Since your Guardium system might not have access to the internet, your plug-in must be created with offline installation in mind. To ensure success, follow the [Logstash documentation](https://www.elastic.co/guide/en/logstash/7.5/offline-plugins.html) . The output is a .zip file with your plug-ins.
  ...
  * On the collector, go to ```Setup > Tools and Views > Configure Univer***REMOVED***l Connector```.
...
```support must_gather univer***REMOVED***l_connector_issues```
...
You can also create and store files in the folder ```$THIRD_PART_PATH```, if your plug-in’s code requires it. For example, to support persistency by keeping track of the last fetched log entry.
  ...
  3. Configure the Guardium univer***REMOVED***l connector to be aware of your data source and use your plug-in, by adding a connection. In the Configure Univer***REMOVED***l Connector page create a Connector configuration. Paste the input and filter sections contents from your Logstash configuration file into the Input and Filter textboxes.
...
* ***If you send multiple, different data sources that use Filebeat from the ***REMOVED***me server:***
...
4. Set the policies you require. Refer to the Policies section of our main univer***REMOVED***l connector documentation for more details. 

5. Enable the Guardium univer***REMOVED***l connector.

When the Guardium univer***REMOVED***l connector runs, it starts a docker container with Logstash in the background. Start the Guardium Univer***REMOVED***l connector by using a API, adding the debug flag to collect more data in the log folder. (By default, only errors are written to the log.) The debug level does not affect the plug-in logger. It shows more details about the data that is passed to the Guardium univer***REMOVED***l connector (output stage), just before it is pushed into Guardium.

* Run: ```grdapi run_univer***REMOVED***l_connector uc_debug_level=debug debug=3```
...
* To change the debug level while the Guardium univer***REMOVED***l connector is running, run

```grdapi set_univer***REMOVED***l_connector_log_level uc_debug_level=debug```

* To stop the Guardium Univer***REMOVED***l connector, use

```grdapi stop_univer***REMOVED***l_connector```

* To check status, use

```grdapi get_univer***REMOVED***l_connector_status```
